...
if 'async function requirePanelPermission(' not in s:
    marker='async function requireBankAdmin(request,env){'
    helper="""async function requirePanelPermission(request,env,permission){
...
"""
    if marker not in s:
        raise SystemExit('marker requireBankAdmin not found')
    s=s.replace(marker,helper+marker,1)

proxy_pattern=r"async function proxySpecializedApi\(request\)\{.*?\n\}\n\nfunction formatDate"
proxy_replacement="""async function proxySpecializedApi(request,env){
...
function formatDate"""
s,n=re.subn(proxy_pattern,proxy_replacement,s,count=1,flags=re.S)
if n!=1:
    raise SystemExit(f'proxySpecializedApi patch count={n}')

service_pattern=r"async function bankProxyAsService\(env,sql,payload\)\{.*?\n\}\n\nfunction nextInvoiceId"
service_replacement="""async function bankProxyAsService(env,sql,payload){
...
function nextInvoiceId"""
s,n=re.subn(service_pattern,service_replacement,s,count=1,flags=re.S)
if n!=1:
    raise SystemExit(f'bankProxyAsService patch count={n}')

old="if(SPECIALIZED_PROXY_PATHS.has(url.pathname))return proxySpecializedApi(request);"
new="if(SPECIALIZED_PROXY_PATHS.has(url.pathname))return proxySpecializedApi(request,env);"
if old not in s:
    raise SystemExit('specialized routing marker not found')
