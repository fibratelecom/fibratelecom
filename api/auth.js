...
const {deriveVaultKey,setVaultCookie,clearVaultCookie,hasVaultCookie}=require('../lib/cloud-vault');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(action==='status'){
      ...
      return res.status(200).json({ok:true,data:{configured:Array.isArray(users)&&users.length>0,authenticated:Boolean(current),vaultReady:Boolean(current&&hasVaultCookie(req)),user:current?.user||null}});
    }
    if(action==='setup'){
      ...
      const created=await db(req,'/pp_users',{method:'POST',headers:{'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify({email:login,name,role:'admin',password_hash:hash})});
      ...
      setVaultCookie(res,deriveVaultKey(password,{id:user.id,password_hash:hash}));
      return res.status(200).json({ok:true,data:{authenticated:true,vaultReady:true,user:{id:user.id,email:user.email,name:user.name,role:user.role}}});
    }
    if(action==='login'){
      ...
      await createSession(req,res,user.id);
      setVaultCookie(res,deriveVaultKey(password,user));
      return res.status(200).json({ok:true,data:{authenticated:true,vaultReady:true,user:{id:user.id,email:user.email,name:user.name,role:user.role}}});
    }
    if(action==='logout'){
      await revokeCurrent(req);
      clearSessionCookie(res);clearVaultCookie(res);
      return res.status(200).json({ok:true,data:{authenticated:false,vaultReady:false}});
    }
