...
async function clientStatus(router,data){
  ...
  const s=active[0]||null,[downloadBytes,uploadBytes]=counterPair(s?.bytes);
  return {online:Boolean(s),checkedAt:new Date().toISOString(),username,sessionId:text(s?.['session-id']||s?.['.id']),ip:text(s?.address||secret?.['remote-address']||data?.ip),callerId:text(s?.['caller-id']||secret?.['caller-id']||data?.mac_address),uptime:text(s?.uptime),encoding:text(s?.encoding),downloadBytes,uploadBytes,downloadBps:0,uploadBps:0,liveRatesAvailable:false,profile:text(secret?.profile||data?.mikrotik_profile),mtu:0,vlan:null,pppoeInterface:'',webAccess:null};
}
