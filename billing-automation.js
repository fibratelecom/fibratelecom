...
.pp-billing-auto-fab{position:fixed;right:18px;bottom:18px;z-index:9000;border:0;border-radius:999px;padding:10px 15px;background:#0d8b78;color:#fff;font:700 12px/1.1 Segoe UI,Arial;box-shadow:0 10px 28px rgba(13,139,120,.25);cursor:pointer}
...
@media(max-width:650px){.pp-billing-auto-clients{grid-template-columns:1fr}.pp-billing-auto-fab{right:12px;bottom:12px}}
...
function mountButton(){
  injectStyle();if(document.querySelector('.pp-billing-auto-fab'))return;
  const button=document.createElement('button');button.className='pp-billing-auto-fab';button.type='button';button.textContent='Mensalidades';button.title='Geração automática, gerar agora e gerar em lote';button.onclick=()=>openManager().catch(error=>toast(error.message||String(error),'error'));document.body.appendChild(button);
}
