...
.pp-billing-auto-body{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.pp-billing-auto-card{padding:17px 18px;border:1px solid #dfe8e5;border-radius:12px;background:#fff}.pp-billing-auto-card:first-child,.pp-billing-negotiation-card{grid-column:1/-1}.pp-billing-auto-card h3{margin:0 0 10px;font-size:15px;color:#2e433d}
...
@media(max-width:900px){.pp-billing-auto-layer{position:fixed;inset:60px 0 0}.pp-billing-auto-page{padding:18px 13px 36px}.pp-billing-auto-head{flex-direction:column}.pp-billing-auto-body{grid-template-columns:1fr}.pp-billing-auto-card:first-child,.pp-billing-negotiation-card{grid-column:auto}}
...
async function openManager(){
  ...
  try{
    ...
    layer.querySelector('.pp-auto-enabled').value=enabled?'true':'false';layer.querySelector('.pp-neg-enabled').value=negotiation.enabled?'true':'false';
    layer.querySelector('.pp-billing-auto-close').onclick=()=>closeManager({goDashboard:true});
