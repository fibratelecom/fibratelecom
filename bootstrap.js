(async()=>{
  ...
  const css=await read(['/parts/basecss-01.txt','/parts/basecss-02.txt','/parts/basecss-03.txt','/parts/fincss-01.txt','/ui-fixes.css?v=1017-cloud3']);
  const style=document.createElement('style');style.textContent=css;document.head.appendChild(style);
  try{const logo=await read(['/packed/logo-01.txt','/packed/logo-02.txt','/packed/logo-03.txt','/packed/logo-04.txt','/packed/logo-05.txt']);const brand=document.createElement('style');brand.textContent=`:root{--fibra-logo:url("data:image/png;base64,${logo.replace(/\s+/g,'')}")}`;document.head.appendChild(brand)}catch(e){console.warn('Logo Fibra+ não carregada',e)}
  ...
  await loadScript('/cloud-adapter.js?v=1017-cloud3');
  ...
  await loadScript('/ui-runtime-fixes.js?v=1017-cloud3');
...
})().catch(err=>{console.error(err);const root=document.getElementById('root')||document.body;root.innerHTML=`<div style="font-family:Segoe UI,Arial,sans-serif;max-width:760px;margin:60px auto;padding:24px;border:1px solid #e5e7eb;border-radius:14px"><h2>Provedor Plus</h2><p>Não foi possível carregar o painel original.</p><pre style="white-space:pre-wrap;color:#b91c1c">${String(err&&err.message||err)}</pre><button onclick="location.reload()">Tentar novamente</button></div>`});
