(async()=>{
  ...
  const finishStartup=()=>{const shell=document.getElementById('pp-startup-shell');if(!shell)return;shell.classList.add('is-done');setTimeout(()=>shell.remove(),220)};
  ...
  await loadScript('/dashboard-transition-guard.js?v=1017-dashboard-guard2-globalnav').catch(error=>console.error('Provedor Plus: a proteção de transição das telas não foi carregada.',error));
