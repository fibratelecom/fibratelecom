...
async function augmentPortalContractContext(response,env,requestBody={},action=''){
  ...
  try{
    ...
    const sql=neon(env.DATABASE_URL),includePlanContext=['login','refresh'].includes(action),planPromise=includePlanContext?planActivity(sql,clientId):Promise.resolve(null),[client,state,planContext]=await Promise.all([portalClient(sql,clientId),loadState(sql),planPromise]);if(!client)return response;const contractId=action==='login'?'primary':text(requestBody?.data?.contractId)||'primary',live=!contractId.startsWith('primary')&&isConnectionAction(action);let scoped=await scopePortalToContract(portal,client,state,env,contractId,live);if(planContext)scoped={...scoped,...planContext};
...
async function auditedBaseFetch(request,env,ctx){
  ...
  const definition=portalAuditDefinition(path,action),baseResponse=await baseWorker.fetch(request,env,ctx);let response=baseResponse;if(path===PORTAL_PATH)response=await augmentPortalContractContext(response,env,body,action);if(!definition||!response.ok||!env.DATABASE_URL)return response;
