(()=>{
  ...
  if(!document.getElementById('pp-dashboard-multirouter-style')){
    const style=document.createElement('style');style.id='pp-dashboard-multirouter-style';style.textContent=`
      ...
      .pp-pppoe-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));align-items:stretch;margin-top:11px;padding-top:11px;border-top:1px solid #edf1f0}
      .pp-pppoe-summary>div{min-width:0;padding:0 10px;border-right:1px solid #edf1f0}
      .pp-pppoe-summary>div:first-child{padding-left:0}.pp-pppoe-summary>div:last-child{padding-right:0;border-right:0}
      .pp-pppoe-summary span{display:block;color:#718184;font-size:10px;line-height:1.35}.pp-pppoe-summary strong{display:block;margin-top:5px;color:#263d39;font-size:17px;line-height:1.2}
      .pp-router-clients-action{display:flex!important;align-items:center;justify-content:center!important}
      .pp-router-clients-button{width:100%;min-height:38px;padding:7px 10px;color:#0d806f;background:#fff;border:1px solid #cfe3de;border-radius:8px;font-size:11px;font-weight:800;line-height:1.25;cursor:pointer}
      .pp-router-clients-button:hover{background:#edf8f5;border-color:#9dcfc4}
      ...
      .pp-pppoe-modal-layer{position:fixed;inset:0;z-index:120;display:grid;place-items:center;padding:18px;background:#0c1d1b73;backdrop-filter:blur(3px)}
      .pp-pppoe-modal{width:min(760px,calc(100vw - 28px));max-height:calc(100vh - 36px);display:flex;flex-direction:column;background:#fff;border:1px solid #dce7e4;border-radius:14px;box-shadow:0 25px 70px #071e1938;overflow:hidden}
      .pp-pppoe-modal-head{display:flex;align-items:flex-start;justify-content:space-between;gap:15px;padding:17px 18px;border-bottom:1px solid #edf1f0}
      .pp-pppoe-modal-head>div{min-width:0}.pp-pppoe-modal-head h3{margin:0;color:#18332e;font-size:17px;line-height:1.3}.pp-pppoe-modal-head p{margin:5px 0 0;color:#718184;font-size:11px;line-height:1.4}
      .pp-pppoe-modal-close{flex:0 0 34px;width:34px;height:34px;border:1px solid #dce5e3;border-radius:8px;background:#fff;color:#647875;font-size:18px;cursor:pointer}
      .pp-pppoe-modal-filters{display:flex;flex-wrap:wrap;gap:8px;padding:12px 18px;border-bottom:1px solid #edf1f0}.pp-pppoe-filter{min-height:34px;padding:0 12px;border:1px solid #dce5e3;border-radius:18px;background:#fff;color:#60726e;font-size:11px;font-weight:750;cursor:pointer}.pp-pppoe-filter.active{color:#fff;background:#0d8b78;border-color:#0d8b78}
      .pp-pppoe-client-list{display:grid;gap:7px;padding:12px 18px 18px;overflow:auto}.pp-pppoe-client-row{display:grid;grid-template-columns:minmax(0,1.5fr) minmax(110px,.7fr) auto;align-items:center;gap:12px;padding:10px 11px;background:#f8faf9;border:1px solid #e8efed;border-radius:9px}.pp-pppoe-client-row>div{display:flex;flex-direction:column;min-width:0}.pp-pppoe-client-row strong{font-size:12px;line-height:1.35;color:#263d39;overflow-wrap:anywhere}.pp-pppoe-client-row small{margin-top:3px;color:#7b8b88;font-size:10px;line-height:1.35;overflow-wrap:anywhere}.pp-pppoe-client-status{display:inline-flex;align-items:center;justify-content:center;min-width:66px;padding:5px 8px;border-radius:18px;font-size:10px;font-weight:800}.pp-pppoe-client-status.online{color:#13785f;background:#e4f5ef}.pp-pppoe-client-status.offline{color:#a85049;background:#fae9e7}.pp-pppoe-client-status.blocked{color:#a66722;background:#faeee1}
      @media(max-width:720px){.pp-router-monitor-head{flex-direction:column}.pp-router-monitor-head>b{align-self:flex-start}.pp-pppoe-summary{grid-template-columns:1fr 1fr}.pp-router-clients-action{grid-column:1/-1;margin-top:10px}.pp-pppoe-client-row{grid-template-columns:1fr}.pp-pppoe-client-status{justify-self:start}}
  ...
  let root=null,refreshing=false,liveRefreshing=false,refreshTimer=null,observerTimer=null,lastData=null,lastLive=null,prevRouterTraffic=new Map();
  ...
  function build(){
    ...
    root.innerHTML=`
      ...
      <div class="pp-network-title"><div><span>MONITORAMENTO</span><h2>Rede e assinantes</h2></div></div>
    ...
    root.querySelector('#pp-dashboard-refresh')?.addEventListener('click',()=>refresh(true));
    root.querySelector('#pp-dashboard-period')?.addEventListener('change',()=>renderRevenue(lastData||collectStateOnly()));
    root.querySelectorAll('[data-nav]').forEach(button=>button.addEventListener('click',()=>navigate(button.dataset.nav||'')));
    root.addEventListener('click',event=>{const button=event.target.closest('[data-pp-router-clients]');if(!button)return;const id=Number(button.dataset.ppRouterClients)||0;const item=lastLive?.routerMetrics?.find(x=>Number(x.routerId)===id);if(item)openClientList(item)});
    setTimeout(()=>refresh(true),30);return root;
  }
  ...
  async function collectLive(clients){
    let routers=[];try{routers=typeof api?.routers?.list==='function'?await api.routers.list():[]}catch(error){return {available:false,error:String(error?.message||error),routers:[],results:[],routerMetrics:[],sessions:[]}}
    routers=Array.isArray(routers)?routers:[];if(!routers.length){prevRouterTraffic=new Map();return {available:true,routers:[],results:[],routerMetrics:[],sessions:[],online:0,offline:0,alerts:0}}
    ...
    const routerMetrics=settled.map(item=>{
      const routerId=Number(item.router?.id)||0,routerName=item.router?.name||`MikroTik ${routerId||''}`.trim(),routerClients=clients.filter(c=>isPppoe(c)&&Number(c.router_id)===routerId),routerSessions=item.ok&&Array.isArray(item.sync?.pppActive)?item.sync.pppActive:[],secrets=item.ok&&Array.isArray(item.sync?.pppSecrets)?item.sync.pppSecrets:[];
      if(item.ok)for(const row of routerSessions)sessions.push({routerId,routerName,...row});
      const onlineNames=new Set(routerSessions.map(s=>String(s?.name||'').trim()).filter(Boolean));
      const secretMap=new Map(secrets.map(s=>[String(s?.name||'').trim(),s]).filter(([name])=>name));
      for(const name of onlineNames)if(!secretMap.has(name))secretMap.set(name,{name,disabled:false,profile:'',remoteAddress:'',callerId:''});
      const appByUser=new Map(routerClients.map(c=>[pppoeUser(c),c]).filter(([name])=>name));
      const users=[...secretMap.values()].map(secret=>{const username=String(secret?.name||'').trim(),session=routerSessions.find(s=>String(s?.name||'').trim()===username)||null,client=appByUser.get(username)||null;return {username,online:onlineNames.has(username),disabled:Boolean(secret?.disabled),profile:String(secret?.profile||session?.profile||''),ip:String(session?.address||secret?.remoteAddress||client?.ip||''),clientName:String(client?.name||''),plan:String(client?.plan_name||client?.plan||'')}}).sort((a,b)=>Number(b.online)-Number(a.online)||a.username.localeCompare(b.username,'pt-BR'));
      const onlineCount=onlineNames.size,offlineCount=Math.max(0,users.length-onlineCount);
      ...
      if(item.ok&&item.sync?.wanTraffic?.available){
        const wan=item.sync.wanTraffic,rx=Math.max(0,num(wan.rxBytes)),tx=Math.max(0,num(wan.txBytes)),prev=prevRouterTraffic.get(routerId),directDown=Number(wan.rxBps),directUp=Number(wan.txBps);
        if(Boolean(wan.rateAvailable)&&Number.isFinite(directDown)&&Number.isFinite(directUp))traffic={available:true,interface:String(wan.interface||''),downloadBps:Math.max(0,Math.round(directDown)),uploadBps:Math.max(0,Math.round(directUp)),reason:''};
        else if(prev&&now>prev.at&&rx>=prev.rx&&tx>=prev.tx){const elapsed=Math.max(.25,(now-prev.at)/1000);traffic={available:true,interface:String(wan.interface||''),downloadBps:Math.round((rx-prev.rx)*8/elapsed),uploadBps:Math.round((tx-prev.tx)*8/elapsed),reason:''}}
        else traffic={available:true,interface:String(wan.interface||''),downloadBps:null,uploadBps:null,reason:'Coletando próxima leitura.'};
        prevRouterTraffic.set(routerId,{rx,tx,at:now});
      }else if(item.ok&&item.sync?.wanTraffic)traffic.reason=String(item.sync.wanTraffic.reason||traffic.reason);
      return {...item,routerId,routerName,pppoe:{online:onlineCount,offline:offlineCount,total:users.length,users},traffic};
    });
    const activeIds=new Set(routerMetrics.map(x=>x.routerId));for(const id of [...prevRouterTraffic.keys()])if(!activeIds.has(id))prevRouterTraffic.delete(id);
  ...
  function renderRouterMonitors(live){
    const pppTarget=root?.querySelector('#pp-pppoe-router-list'),trafficTarget=root?.querySelector('#pp-traffic-router-list'),metrics=Array.isArray(live?.routerMetrics)?live.routerMetrics:[];
    if(pppTarget){
      if(!metrics.length)pppTarget.innerHTML='<div class="pp-empty-inline">Nenhum MikroTik cadastrado.</div>';
      else pppTarget.innerHTML=metrics.map(item=>`<article class="pp-router-monitor-item"><div class="pp-router-monitor-head"><div><strong>${esc(item.routerName)}</strong><small>${item.ok?'RouterOS conectado':esc(item.error||'Sem resposta')}</small></div>${item.ok?'':`<b class="bad">Offline</b>`}</div><div class="pp-pppoe-summary"><div><span>Clientes Online</span><strong>${item.ok?integer(item.pppoe.online):'—'}</strong></div><div><span>Clientes Offline</span><strong>${item.ok?integer(item.pppoe.offline):'—'}</strong></div><div class="pp-router-clients-action"><button type="button" class="pp-router-clients-button" data-pp-router-clients="${item.routerId}" ${item.ok?'':'disabled'}>Ver clientes</button></div></div></article>`).join('');
    }
    if(trafficTarget){
      if(!metrics.length)trafficTarget.innerHTML='<div class="pp-empty-inline">Nenhum MikroTik cadastrado.</div>';
      else trafficTarget.innerHTML=metrics.map(item=>{const traffic=item.traffic||{};if(!item.ok)return `<article class="pp-router-monitor-item pp-router-traffic-item"><div class="pp-router-monitor-head"><div><strong>${esc(item.routerName)}</strong><small>MikroTik sem resposta</small></div><b class="bad">Offline</b></div><div class="pp-traffic-unavailable">Tráfego indisponível</div></article>`;if(!traffic.available)return `<article class="pp-router-monitor-item pp-router-traffic-item"><div class="pp-router-monitor-head"><div><strong>${esc(item.routerName)}</strong><small>${esc(traffic.interface?`Interface ${traffic.interface}`:'Interface de saída')}</small></div><b class="warn">Indisponível</b></div><div class="pp-traffic-unavailable">${esc(traffic.reason||'Não foi possível identificar a interface de saída do MikroTik.')}</div></article>`;if(traffic.downloadBps==null||traffic.uploadBps==null)return `<article class="pp-router-monitor-item pp-router-traffic-item"><div class="pp-router-monitor-head"><div><strong>${esc(item.routerName)}</strong><small>Interface ${esc(traffic.interface||'WAN')}</small></div><b>Coletando</b></div><div class="pp-traffic-gauges"><div class="pp-traffic-gauge-item"><div class="pp-traffic-ring" style="--pp-ring-deg:65deg"><b>…</b><small>COLETANDO</small></div><span>Download atual</span></div><div class="pp-traffic-gauge-item"><div class="pp-traffic-ring pp-traffic-ring-up" style="--pp-ring-deg:65deg"><b>…</b><small>COLETANDO</small></div><span>Upload atual</span></div></div><small class="pp-traffic-peak">Aguardando leitura do MikroTik.</small></article>`;const peak=peakStore(item.routerId,traffic.downloadBps,traffic.uploadBps);return `<article class="pp-router-monitor-item pp-router-traffic-item"><div class="pp-router-monitor-head"><div><strong>${esc(item.routerName)}</strong><small>Interface ${esc(traffic.interface||'WAN')}</small></div></div><div class="pp-traffic-gauges">${trafficRing(traffic.downloadBps,peak.down,'down')}${trafficRing(traffic.uploadBps,peak.up,'up')}</div><small class="pp-traffic-peak">Pico observado hoje: ↓ ${esc(formatBps(peak.down))} • ↑ ${esc(formatBps(peak.up))}</small></article>`}).join('');
    }
  }

  function openClientList(item){
    document.querySelector('.pp-pppoe-modal-layer')?.remove();
    const layer=document.createElement('div');layer.className='pp-pppoe-modal-layer';
    const users=Array.isArray(item?.pppoe?.users)?item.pppoe.users:[];
    layer.innerHTML=`<section class="pp-pppoe-modal" role="dialog" aria-modal="true"><div class="pp-pppoe-modal-head"><div><h3>Clientes PPPoE — ${esc(item.routerName)}</h3><p>${integer(item.pppoe.online)} online • ${integer(item.pppoe.offline)} offline • ${integer(item.pppoe.total)} total</p></div><button type="button" class="pp-pppoe-modal-close" aria-label="Fechar">×</button></div><div class="pp-pppoe-modal-filters"><button type="button" class="pp-pppoe-filter active" data-filter="all">Todos (${integer(item.pppoe.total)})</button><button type="button" class="pp-pppoe-filter" data-filter="online">Online (${integer(item.pppoe.online)})</button><button type="button" class="pp-pppoe-filter" data-filter="offline">Offline (${integer(item.pppoe.offline)})</button></div><div class="pp-pppoe-client-list"></div></section>`;
    const list=layer.querySelector('.pp-pppoe-client-list');
    const draw=filter=>{const selected=filter==='online'?users.filter(u=>u.online):filter==='offline'?users.filter(u=>!u.online):users;list.innerHTML=selected.length?selected.map(user=>`<article class="pp-pppoe-client-row"><div><strong>${esc(user.clientName||user.username||'Cliente')}</strong><small>${user.clientName?`PPPoE: ${esc(user.username)}`:esc(user.username)}</small></div><div><strong>${esc(user.ip||'Sem IP')}</strong><small>${esc(user.profile||'Sem perfil')}</small></div><span class="pp-pppoe-client-status ${user.online?'online':user.disabled?'blocked':'offline'}">${user.online?'Online':user.disabled?'Bloqueado':'Offline'}</span></article>`).join(''):'<div class="pp-empty-inline">Nenhum cliente neste filtro.</div>'};
    draw('all');
    layer.querySelectorAll('.pp-pppoe-filter').forEach(button=>button.addEventListener('click',()=>{layer.querySelectorAll('.pp-pppoe-filter').forEach(x=>x.classList.remove('active'));button.classList.add('active');draw(button.dataset.filter||'all')}));
    const close=()=>layer.remove();layer.querySelector('.pp-pppoe-modal-close')?.addEventListener('click',close);layer.addEventListener('click',event=>{if(event.target===layer)close()});
    document.body.appendChild(layer);
  }

  function renderLive(live,clients){
    if(!root)return;if(!live?.available){['pp-health-percent','pp-health-ring-text','pp-health-online','pp-health-offline','pp-health-alerts'].forEach(id=>setText(id,'Indisponível'));setText('pp-health-detail','Não foi possível consultar os MikroTik');const p=root.querySelector('#pp-pppoe-router-list'),t=root.querySelector('#pp-traffic-router-list');if(p)p.innerHTML='<div class="pp-empty-inline">MikroTik indisponível.</div>';if(t)t.innerHTML='<div class="pp-empty-inline">Tráfego indisponível.</div>';return}
    const total=live.routers.length,pct=total?live.online/total*100:null;setText('pp-health-percent',pct==null?'Sem MikroTik':percent(pct));setText('pp-health-ring-text',pct==null?'—':`${Math.round(pct)}%`);setText('pp-health-detail',total?`${live.online} de ${total} roteador${total===1?'':'es'} respondendo`:'Nenhum MikroTik cadastrado');setText('pp-health-online',integer(live.online));setText('pp-health-offline',integer(live.offline));setText('pp-health-alerts',integer(live.alerts));const ring=root.querySelector('#pp-health-ring');if(ring)ring.style.setProperty('--pp-ring-deg',`${Math.max(0,Math.min(360,(pct||0)*3.6))}deg`);
  ...
  function renderConsumption(live,clients){
    const target=root?.querySelector('#pp-consumption-ranking');if(!target)return;setText('pp-consumption-subtitle','Consumo das sessões PPPoE atuais');
    const byKey=new Map();
    for(const session of live?.sessions||[]){const username=String(session?.name||'').trim();if(!username)continue;const pair=activeBytes(session),bytes=pair.down+pair.up;if(bytes<=0)continue;const key=`${Number(session.routerId)||0}|${username}`,existing=byKey.get(key)||{routerId:Number(session.routerId)||0,routerName:String(session.routerName||'MikroTik'),username,bytes:0};existing.bytes+=bytes;byKey.set(key,existing)}
    const rows=[...byKey.values()].map(row=>{const client=clients.find(c=>Number(c.router_id)===row.routerId&&pppoeUser(c)===row.username)||null;return {...row,client}}).sort((a,b)=>b.bytes-a.bytes).slice(0,5),max=rows[0]?.bytes||0;
    if(!rows.length){target.innerHTML='<div class="pp-empty-inline">Nenhum consumo PPPoE disponível neste momento.</div>';return}
    target.innerHTML=rows.map((row,index)=>`<article><span class="pp-rank-number">${index+1}</span><div class="pp-rank-main"><div><strong>${esc(row.client?.name||row.username)}</strong><small>${esc(row.client?.plan_name||row.client?.plan||`${row.routerName} • PPPoE ${row.username}`)}</small></div><i><b style="width:${max?Math.max(5,row.bytes/max*100):0}%"></b></i></div><strong class="pp-rank-value">${esc(formatBytes(row.bytes))}</strong></article>`).join('');
  }
  ...
  function updateTimestamp(){const now=new Date();setText('pp-dashboard-updated',`Atualizado às ${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`)}
  async function refreshLive(){if(liveRefreshing||refreshing||!dashboardActive()||!lastData)return;liveRefreshing=true;try{const live=await collectLive(lastData.clients);lastLive=live;renderLive(live,lastData.clients);updateTimestamp()}catch(error){console.error('Provedor Plus: falha ao atualizar monitoramento em tempo real.',error)}finally{liveRefreshing=false}}
  async function refresh(){if(refreshing||!dashboardActive())return;const view=build();if(!view)return;refreshing=true;const button=root.querySelector('#pp-dashboard-refresh');if(button)button.disabled=true;try{const data=await collectBase();lastData=data;renderMetrics(data);renderRevenue(data);renderPlans(data);renderRecent(data);const live=await collectLive(data.clients);lastLive=live;renderLive(live,data.clients);updateTimestamp()}catch(error){console.error('Provedor Plus: falha ao atualizar o Dashboard.',error);setText('pp-dashboard-updated','Falha ao atualizar alguns indicadores')}finally{refreshing=false;if(button)button.disabled=false}}

  function schedule(){clearTimeout(observerTimer);observerTimer=setTimeout(()=>{if(dashboardActive()){build();if(!refreshTimer)refreshTimer=setInterval(refreshLive,3000)}else{document.querySelector('.page-wrap')?.classList.remove('pp-dashboard-host');root=null;document.querySelector('.pp-pppoe-modal-layer')?.remove();if(refreshTimer){clearInterval(refreshTimer);refreshTimer=null}}},80)}
