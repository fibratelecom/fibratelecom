...
function invoiceKey(invoice){const raw=text(invoice?.id),n=Number(raw);return Number.isFinite(n)&&raw?`n:${String(Math.trunc(n)).padStart(20,'0')}`:`s:${raw}`}
function selectBankBatch(pending,cursorValue){
  const sorted=[...(pending||[])].sort((a,b)=>invoiceKey(a).localeCompare(invoiceKey(b))),cursor=text(cursorValue?.last_key);let start=0;
  if(cursor){const found=sorted.findIndex(item=>invoiceKey(item)>cursor);start=found<0?0:found}
  const batch=sorted.slice(start,start+BANK_BATCH_SIZE),more=start+batch.length<sorted.length,lastKey=more&&batch.length?invoiceKey(batch[batch.length-1]):'';
  return {batch,more,lastKey,total:sorted.length};
}
...
async function runAutomation(req){
  const startedAt=new Date().toISOString(),skipBank=bool(req?.query?.skipBank,false),[stateRow,overlayRow,cursorRow,credentials,clientsRows,routersRows,passwords]=await Promise.all([
    settingGet(req,STATE_KEY),settingGet(req,OVERLAY_KEY),settingGet(req,BANK_CURSOR_KEY),bankVault(req),db(req,'/pp_clients?select=*&order=id.asc'),db(req,'/pp_routers?select=*&order=id.asc'),routerPasswordMap(req)
  ]);
  ...
  let checked=0,paid=0,bankFailed=0,overlayDirty=false,morePending=false,bankBatchSize=0;
  const pending=(state.invoices||[]).filter(i=>i.status==='Pendente'&&['efi','mercadoPago'].includes(text(i.bank_provider))),selection=skipBank?{batch:[],more:false,lastKey:text(cursorRow?.value?.last_key),total:pending.length}:selectBankBatch(pending,cursorRow?.value||{}),bankBatch=selection.batch;
  bankBatchSize=bankBatch.length;morePending=selection.more;
  if(bankBatch.length){
    const bankResults=await mapConcurrent(bankBatch,4,async invoice=>{
      ...
      if(remote&&typeof remote==='object'){overlay.invoices[String(invoice.id)]={...(overlay.invoices[String(invoice.id)]||{}),...clone(remote)};overlayDirty=true}
      if(paidState(invoice,remote)){
        ...
        overlay.invoices[String(invoice.id)]=patch;overlayDirty=true;paid++;
      }
      return remote;
    });
  ...
  if(!skipBank){
    if(overlayDirty){overlay.version=1;overlay.updated_at=new Date().toISOString();await settingSet(req,OVERLAY_KEY,overlay);overlayDirty=false}
    await settingSet(req,BANK_CURSOR_KEY,{last_key:selection.lastKey||'',pending_total:selection.total,batch_size:bankBatchSize,updated_at:new Date().toISOString()});
  }

  const effectiveState=applyAutomationOverlay(state,overlay),globalEnabled=text(effectiveState?.settings?.auto_block||'false').toLowerCase()==='true',restoreCandidates=[],blockCandidates=[];
  for(const client of effectiveClients){
    ...
    const overdue7=hasOverdue(effectiveState,id,AUTO_BLOCK_DAYS),automaticEnabled=globalEnabled||bool(client.auto_block,false),autoRecord=overlay.clients[String(id)]||{},job={id,client,router:routerObject(routerRow,password),autoRecord};
    if(text(client.status)==='Bloqueado'&&!overdue7&&text(autoRecord.automation_reason)==='overdue_7d'){restoreCandidates.push(job);continue}
    if(automaticEnabled&&text(client.status)!=='Bloqueado'&&!trustActive(client)&&overdue7)blockCandidates.push(job);
  }
  const accessCandidates=[...restoreCandidates,...blockCandidates],accessBatch=accessCandidates.slice(0,ACCESS_BATCH_SIZE),moreAccess=accessCandidates.length>accessBatch.length;
  let blocked=0,unblocked=0,mikrotikFailed=0;
  const accessResults=await mapConcurrent(accessBatch,4,async job=>{
    const {id,client,router,autoRecord}=job,restore=restoreCandidates.includes(job),stamp=new Date().toISOString();
    if(restore){
      const remote=await mikrotikAction('client.unblock',router,client);
      try{
        const nextStatus=hasAnyOverdue(effectiveState,id)?'Em atraso':'Ativo';
        await patchClient(req,id,{status:nextStatus,mikrotik_secret_id:text(remote?.secretId),mikrotik_status:'Sincronizado',mikrotik_last_sync:stamp});
        overlay.clients[String(id)]={...autoRecord,status:nextStatus,mikrotik_secret_id:text(remote?.secretId),mikrotik_status:'Sincronizado',mikrotik_last_sync:stamp,automation_reason:'',automation_at:stamp};overlayDirty=true;unblocked++;
      }catch(error){try{await mikrotikAction('client.block',router,client)}catch{}throw error}
      return;
    }
    const remote=await mikrotikAction('client.block',router,client);
    try{
      await patchClient(req,id,{status:'Bloqueado',mikrotik_secret_id:text(remote?.secretId),mikrotik_status:'Bloqueado no MikroTik',mikrotik_last_sync:stamp});
      overlay.clients[String(id)]={...autoRecord,status:'Bloqueado',mikrotik_secret_id:text(remote?.secretId),mikrotik_status:'Bloqueado no MikroTik',mikrotik_last_sync:stamp,automation_reason:'overdue_7d',automation_at:stamp};overlayDirty=true;blocked++;
    }catch(error){try{await mikrotikAction('client.unblock',router,client)}catch{}throw error}
  });
  mikrotikFailed=accessResults.filter(x=>!x.ok).length;
  if(overlayDirty){overlay.version=1;overlay.updated_at=new Date().toISOString();await settingSet(req,OVERLAY_KEY,overlay)}
  const result={ok:true,startedAt,finishedAt:new Date().toISOString(),pending:pending.length,bankBatch:bankBatchSize,checked,paid,bankFailed,morePending,accessQueued:accessCandidates.length,accessBatch:accessBatch.length,moreAccess,blocked,unblocked,mikrotikFailed,autoBlockEnabled:globalEnabled,skipBank};
