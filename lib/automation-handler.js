...
async function bankVault(req){
  const [settings,users]=await Promise.all([db(req,`/pp_settings?key=like.${encodeURIComponent(BANK_SECRET_PREFIX+'*')}&select=key,value,updated_at&order=updated_at.desc`),usersById(req)]);
  for(const row of Array.isArray(settings)?settings:[]){
    ...
    try{const value=JSON.parse(plain);if(value&&typeof value==='object')return value}catch{}
  }
  return {};
}
