...
function clientPayload(data={}){
  ...
  if(id)out.id=id;
  if(text(data.pppoe_password))out.pppoe_password=text(data.pppoe_password);
  return out;
}
...
async function bankAutomationContext(req){
  const current=await currentSession(req);
  if(!current?.user?.id)throw Object.assign(new Error('Sessão expirada ou não autenticada.'),{statusCode:401});
  const users=await db(req,`/pp_users?id=eq.${Number(current.user.id)}&select=password_hash&limit=1`);
  const passwordHash=Array.isArray(users)?text(users[0]?.password_hash):'';
  if(!passwordHash)throw Object.assign(new Error('Não foi possível proteger a credencial bancária da automação.'),{statusCode:500});
  const userId=Number(current.user.id),key=crypto.createHash('sha256').update(`provedor-plus-bank-automation-v1|${userId}|${passwordHash}`).digest();
  return {userId,key,settingKey:`automation_bank_secret_v1_${userId}`};
}
...
async function bankAutomationSave(req,data={}){
  const provider=text(data.provider),credentials=data.credentials&&typeof data.credentials==='object'?data.credentials:{};
  if(!['efi','mercadoPago'].includes(provider))throw Object.assign(new Error('Banco inválido para a automação.'),{statusCode:400});
  const ctx=await bankAutomationContext(req),row=await settingGet(req,ctx.settingKey),plain=decryptSecret(row?.value,ctx.key);let vault={};
  try{vault=plain?JSON.parse(plain):{}}catch{vault={}}
  if(!vault||typeof vault!=='object'||Array.isArray(vault))vault={};
  if(provider==='efi'){
    vault.efi={
      clientId:text(credentials.clientId),clientSecret:text(credentials.clientSecret),
      certificatePassword:String(credentials.certificatePassword||''),certificateBase64:String(credentials.certificateBase64||''),certificateName:text(credentials.certificateName)
    };
  }else vault.mercadoPago={accessToken:text(credentials.accessToken)};
  await settingSet(req,ctx.settingKey,encryptSecret(JSON.stringify(vault),ctx.key));
  return provider==='efi'?{provider,configured:Boolean(vault.efi.clientId&&vault.efi.clientSecret),certificateConfigured:Boolean(vault.efi.certificateBase64)}:{provider,configured:Boolean(vault.mercadoPago.accessToken)};
}
...
async function trafficRecord(req,data={}){
  ...
  const month=currentMonth(data.month),key=`client_traffic_v1_${clientId}`,row=await settingGet(req,key),all=row?.value&&typeof row.value==='object'?row.value:trafficEmpty(month);
  const live=data.live&&typeof data.live==='object'?data.live:{},t=Date.now();
  let x={...trafficEmpty(month),...all};
  if(x.month!==month){
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    else if(action==='automation.bank.save')result=await bankAutomationSave(req,data);
