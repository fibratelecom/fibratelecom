...
async function activeSessions(router,username=''){
  ...
  if(!list.length){
    try{const all=rows(await request(router,'ppp/active'));list=username?all.filter(x=>text(x?.name)===username):all}catch{}
  }
...
async function readWanTraffic(router){
  ...
  if(!interfaces.length){
    try{interfaces=rows(await request(router,'interface'))}catch(error){return {available:false,interface:'',rxBytes:0,txBytes:0,reason:error instanceof Error?error.message:String(error)}}
  }
  let routes=[];
  ...
  let interfaceName='';
  for(const route of candidates){interfaceName=routeInterface(route,interfaces);if(interfaceName)break}
  if(!interfaceName)return {available:false,interface:'',rxBytes:0,txBytes:0,reason:'Não foi possível identificar a interface de saída pela rota padrão do MikroTik.'};
  let iface=interfaces.find(i=>text(i?.name)===interfaceName)||null;
  if(!iface){
    try{iface=(await print(router,'interface',['.id','name','type','running','disabled','rx-byte','tx-byte'],[`name=${interfaceName}`],{stats:''}))[0]||null}catch{}
  }
  if(!iface)return {available:false,interface:interfaceName,rxBytes:0,txBytes:0,reason:`A interface de saída ${interfaceName} não foi encontrada.`};
  const rx=Number(iface?.['rx-byte']),tx=Number(iface?.['tx-byte']);
  if(!Number.isFinite(rx)||!Number.isFinite(tx))return {available:false,interface:interfaceName,rxBytes:0,txBytes:0,reason:`O RouterOS não retornou os contadores de tráfego da interface ${interfaceName}.`};
  return {available:true,interface:interfaceName,interfaceType:text(iface?.type),running:boolValue(iface?.running),rxBytes:Math.max(0,rx),txBytes:Math.max(0,tx)};
}
async function snapshot(router){
  const resource=await request(router,'system/resource');
  const [pppResult,trafficResult]=await Promise.allSettled([activeSessions(router),readWanTraffic(router)]);
  const pppActive=pppResult.status==='fulfilled'?pppResult.value:[],warning=pppResult.status==='rejected'?String(pppResult.reason?.message||pppResult.reason):null;
  const wanTraffic=trafficResult.status==='fulfilled'?trafficResult.value:{available:false,interface:'',rxBytes:0,txBytes:0,reason:String(trafficResult.reason?.message||trafficResult.reason||'Tráfego indisponível.')};
  return {connected:true,connectionMethod:'rest',resource,pppActive,wanTraffic,lastSync:new Date().toISOString(),warning};
}
