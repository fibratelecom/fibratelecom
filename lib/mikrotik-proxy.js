...
async function print(router,path,proplist=[],query=[]){
  const body={};if(proplist.length)body['.proplist']=proplist;if(query.length)body['.query']=query;
...
async function snapshot(router){
  ...
  try{pppActive=await request(router,'ppp/active');if(!Array.isArray(pppActive))pppActive=[]}catch(e){warning=e.message}
...
function counterPair(v){const [a='0',b='0']=String(v||'0/0').split('/');return [Math.max(0,Number(a)||0),Math.max(0,Number(b)||0)]}
...
async function clientStatus(router,data){
  ...
  const [active,secret]=await Promise.all([print(router,'ppp/active',['.id','name','service','caller-id','address','uptime','encoding','bytes','session-id'],[`name=${username}`]),findSecret(router,username)]);
  const s=active[0]||null,[downloadBytes,uploadBytes]=counterPair(s?.bytes),ip=text(s?.address||secret?.['remote-address']||data?.ip);
  const quality=s?await pingQuality(router,ip):{qualityAvailable:false,latencyMs:0,packetLoss:null,quality:'Sem conexão'};
  return {online:Boolean(s),checkedAt:new Date().toISOString(),username,sessionId:text(s?.['session-id']||s?.['.id']),ip,callerId:text(s?.['caller-id']||secret?.['caller-id']||data?.mac_address),uptime:text(s?.uptime),encoding:text(s?.encoding),downloadBytes,uploadBytes,downloadBps:0,uploadBps:0,liveRatesAvailable:false,profile:text(secret?.profile||data?.mikrotik_profile),mtu:0,vlan:null,pppoeInterface:'',webAccess:null,...quality};
}
async function blockClient(router,data){
  ...
  return {action:'blocked',secretId:id,username};
}
async function unblockClient(router,data){
  ...
  return {action:'unblocked',secretId:id,username,profile};
}
