...
async function pppoeTopology(router,username,callerId=''){
  ...
  try{const list=await print(router,'interface/pppoe-server',['.id','name','user','service','caller-id','uptime','encoding','mtu','mru','local-address','remote-address','interface','parent-interface','vlan-id','vid'],[`user=${username}`]);dynamic=list[0]||null;if(!dynamic){const all=await print(router,'interface/pppoe-server',['.id','name','user','service','caller-id','uptime','encoding','mtu','mru','local-address','remote-address','interface','parent-interface','vlan-id','vid']);dynamic=all.find(x=>text(x?.user)===username)||null}}catch{}
  ...
  const hostVid=numberValue(host?.vid)||0,dynamicVid=numberValue(dynamic?.['vlan-id'],dynamic?.vid)||0,dynamicService=text(dynamic?.service||dynamic?.['service-name']);
  let server=dynamicService?servers.find(x=>text(x?.['service-name'])===dynamicService):null;
  const dynamicParent=text(dynamic?.interface||dynamic?.['parent-interface']);
  ...
  if(!vlan){for(const candidate of [dynamicParent,serverInterface,text(host?.['on-interface'])].filter(Boolean)){vlan=vlans.find(x=>text(x?.name)===candidate)||null;if(vlan)break}}
  let vlanId=hostVid||dynamicVid||numberValue(vlan?.['vlan-id'])||0;
  const hostPort=text(host?.['on-interface']);
  if(!vlanId&&hostPort){
  ...
  const accessPort=hostPort||text(vlan?.interface)||dynamicParent||serverInterface;
...
async function clientStatus(router,data){
  ...
  const rate=s&&topology.pppoeInterface?await monitorInterfaceTraffic(router,topology.pppoeInterface):{available:false,rxBps:null,txBps:null};
  const downloadBps=rate.available?Math.max(0,Number(rate.txBps)||0):0,uploadBps=rate.available?Math.max(0,Number(rate.rxBps)||0):0;
  return {online:Boolean(s),checkedAt:new Date().toISOString(),username,sessionId:text(s?.['session-id']||s?.['.id']),ip,callerId,uptime:text(s?.uptime),encoding:text(topology.encoding||s?.encoding),downloadBytes,uploadBytes,downloadBps,uploadBps,liveRatesAvailable:rate.available,profile:text(secret?.profile||data?.mikrotik_profile),mtu:numberValue(topology.mtu)||0,mru:numberValue(topology.mru)||0,vlan:topology.vlan??null,vlanId:topology.vlanId??null,pppoeInterface:text(topology.pppoeInterface),accessPort:text(topology.accessPort),accessInterface:text(topology.accessInterface),serverInterface:text(topology.serverInterface),serviceName:text(topology.serviceName),bridge:text(topology.bridge),webAccess:null,...quality};
}
