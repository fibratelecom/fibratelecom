...
function counterPair(value,row={}){
  ...
  if(raw.includes('/')){
    const [a='0',b='0']=raw.split('/');
    return [Math.max(0,Number(a)||0),Math.max(0,Number(b)||0)];
  }
...
function normalizeActiveRow(row={}){
  const [downloadBytes,uploadBytes]=counterPair(row?.bytes,row);
  return {...row,downloadBytes,uploadBytes,bytes:`${downloadBytes}/${uploadBytes}`};
}
async function activeSessions(router,username=''){
  ...
  if(!list.length){
    try{list=await print(router,'ppp/active',props,query)}catch{}
  }
  if(!list.length){
    try{
...
async function snapshot(router){
  ...
  let pppActive=[];let warning=null;
  try{pppActive=await activeSessions(router)}catch(e){warning=e.message}
  return {connected:true,connectionMethod:'rest',resource,pppActive,lastSync:new Date().toISOString(),warning};
}
...
async function savePppoe(router,data){
  ...
  const found=await findSecret(router,username);
  if(!found&&!password)throw Error('Informe a senha PPPoE para criar o acesso.');
...
async function pingQuality(router,address){
  const empty={qualityAvailable:false,latencyMs:0,packetLoss:null,quality:'Indisponível'};
  if(!text(address))return empty;
  try{
    const result=await request(router,'ping',{method:'POST',body:{address:text(address),count:'3',interval:'200ms'}}),list=rows(result);
    let loss=NaN,latency=NaN;
    const summary=list.find(x=>x&&typeof x==='object'&&(x['packet-loss']!=null||x['avg-rtt']!=null||x.avg!=null));
    if(summary){
      const parsedLoss=Number(String(summary['packet-loss']??'').replace('%',''));if(Number.isFinite(parsedLoss))loss=parsedLoss;
      latency=durationMs(summary['avg-rtt']??summary.avg);
    }
    const times=list.map(x=>durationMs(x?.time)).filter(Number.isFinite);
    if(!Number.isFinite(latency)&&times.length)latency=times.reduce((a,b)=>a+b,0)/times.length;
    if(!Number.isFinite(loss))loss=Math.max(0,Math.min(100,Math.round((1-Math.min(3,times.length)/3)*100)));
    if(!Number.isFinite(latency))return {...empty,packetLoss:loss};
    const quality=loss>=20||latency>150?'Ruim':loss>0||latency>80?'Atenção':'Boa';
    return {qualityAvailable:true,latencyMs:Math.round(latency),packetLoss:Math.round(loss),quality};
...
async function pppoeTopology(router,username,callerId=''){
  ...
  try{
    const list=await print(router,'interface/pppoe-server',['.id','name','user','service','caller-id','uptime','encoding','mtu','mru','local-address','remote-address'],[`user=${username}`]);
    dynamic=list[0]||null;
    if(!dynamic){const all=await print(router,'interface/pppoe-server',['.id','name','user','service','caller-id','uptime','encoding','mtu','mru','local-address','remote-address']);dynamic=all.find(x=>text(x?.user)===username)||null}
  }catch{}
  ...
  const hostVid=numberValue(host?.vid)||0;
  const dynamicService=text(dynamic?.service||dynamic?.['service-name']);
  let server=dynamicService?servers.find(x=>text(x?.['service-name'])===dynamicService):null;
  let vlan=hostVid?vlans.find(x=>Number(x?.['vlan-id'])===hostVid):null;
  if(!server&&vlan)server=servers.find(x=>text(x?.interface)===text(vlan?.name))||null;
  if(!server&&servers.length===1)server=servers[0];
  const serverInterface=text(server?.interface);
  if(!vlan&&serverInterface)vlan=vlans.find(x=>text(x?.name)===serverInterface)||null;
  const vlanId=hostVid||numberValue(vlan?.['vlan-id'])||0;
  const accessPort=text(host?.['on-interface']||vlan?.interface||serverInterface);
  const mtu=numberValue(dynamic?.mtu,server?.['max-mtu'])||0;
  return {
    pppoeInterface:text(dynamic?.name),
    accessPort,
    accessInterface:accessPort,
    serverInterface,
    vlan:vlanId||null,
    vlanId:vlanId||null,
    encoding:text(dynamic?.encoding),
    mtu,
    mru:numberValue(dynamic?.mru,server?.['max-mru'])||0,
    serviceName:text(server?.['service-name']||dynamicService),
    bridge:text(host?.bridge),
  };
}
async function clientStatus(router,data){
  ...
  const [quality,topology]=await Promise.all([
    s?pingQuality(router,ip):Promise.resolve({qualityAvailable:false,latencyMs:0,packetLoss:null,quality:'Sem conexão'}),
    s?pppoeTopology(router,username,callerId):Promise.resolve({pppoeInterface:'',accessPort:'',accessInterface:'',serverInterface:'',vlan:null,vlanId:null,encoding:'',mtu:0,mru:0,serviceName:'',bridge:''})
  ]);
...
function isDisabled(value){return ['true','yes','1','on'].includes(text(value).toLowerCase())}
async function verifyBlocked(router,username){
  const [secret,active]=await Promise.all([findSecret(router,username),activeSessions(router,username)]);
  if(!secret||!isDisabled(secret.disabled))throw Error('O MikroTik não confirmou o bloqueio do acesso PPPoE.');
  if(active.length)throw Error('O acesso foi marcado como bloqueado, mas a sessão PPPoE ainda está ativa.');
  return true;
}
async function verifyUnblocked(router,username){
  const secret=await findSecret(router,username);
  if(!secret)throw Error('Acesso PPPoE não encontrado no MikroTik.');
  if(isDisabled(secret.disabled))throw Error('O MikroTik não confirmou a liberação do acesso PPPoE.');
  return true;
}
async function blockClient(router,data){
  const username=text(data?.pppoe_username);if(!username)throw Error('Este cliente não possui usuário PPPoE.');
  let found=await findSecret(router,username);if(!found){await savePppoe(router,{...data,status:'Ativo'});found=await findSecret(router,username)}
  const id=text(found?.['.id']);if(!id)throw Error('Acesso PPPoE não encontrado no MikroTik.');
  await request(router,`ppp/secret/${encodeURIComponent(id)}`,{method:'PATCH',body:{disabled:'true',comment:`Provedor Plus - BLOQUEADO - INADIMPLÊNCIA - ${text(data?.name)||username}`}});await disconnect(router,username);
  await verifyBlocked(router,username);
  return {action:'blocked',secretId:id,username,verified:true};
}
async function unblockClient(router,data){
  const username=text(data?.pppoe_username);if(!username)throw Error('Este cliente não possui usuário PPPoE.');
  const found=await findSecret(router,username);if(!found)throw Error('Acesso PPPoE não encontrado no MikroTik.');
  const id=text(found['.id']),profile=text(data?.mikrotik_profile)||text(found.profile)||'default';
  await request(router,`ppp/secret/${encodeURIComponent(id)}`,{method:'PATCH',body:{disabled:'false',profile,comment:`Provedor Plus - ${text(data?.name)||username}`}});
  await verifyUnblocked(router,username);
  return {action:'unblocked',secretId:id,username,profile,verified:true};
}
async function remoteInfo(router){
  const resource=await request(router,'system/resource');let cloud={};try{const r=await request(router,'ip/cloud/print',{method:'POST',body:{'.proplist':['dns-name','ddns-enabled','status','public-address']}});cloud=Array.isArray(r)?r[0]||{}:r||{}}catch{}
  return {enabled:true,status:'REST HTTPS',dnsName:text(cloud['dns-name'])||router.host,apiPrepared:true,architecture:text(resource?.['architecture-name']),version:text(resource?.version),routerHost:router.host,mode:'cloud-rest',clientConfig:''};
}
