...
async function customer(sql,id){const result=await sql.prepare('SELECT id,name,status,plan_id,plan,city,state FROM pp_clients WHERE id = ? LIMIT 1').bind(Number(id)).all();return result?.results?.[0]||null}
