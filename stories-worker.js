...
async function readStore(env){if(!env.PROVEDOR_DB)throw Object.assign(new Error('Banco D1 não configurado.'),{statusCode:503});const sql=env.PROVEDOR_DB,result=await sql.prepare('SELECT value FROM pp_settings WHERE key = ? LIMIT 1').bind(STORE_KEY).all(),parsed=parseStore(result?.results?.[0]?.value);return {sql,store:{...emptyStore(),...parsed,stories:Array.isArray(parsed?.stories)?parsed.stories:[]}}}
async function writeStore(sql,store){const at=new Date().toISOString(),raw=JSON.stringify({...store,version:1});await sql.prepare('INSERT INTO pp_settings (key,value,updated_at) VALUES (?, ?, ?) ON CONFLICT (key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at').bind(STORE_KEY,raw,at).run();return at}
...
async function customer(env,id){if(!env.DATABASE_URL)throw Object.assign(new Error('Conexão atual de clientes não configurada.'),{statusCode:503});const sql=neon(env.DATABASE_URL),rows=await sql`SELECT id,name,status,plan_id,plan,city,state FROM pp_clients WHERE id=${Number(id)} LIMIT 1`;return rows?.[0]||null}
...
function publicStory(story,clientId=0){const mediaType=story.mediaType==='video'?'video':story.mediaType==='text'?'text':'image';return {id:text(story.id),title:text(story.title),message:text(story.message),mediaType,mediaUrl:mediaType==='text'?'':text(story.mediaUrl),backgroundStyle:['brand','orange','dark','light'].includes(text(story.backgroundStyle))?text(story.backgroundStyle):'brand',actionLabel:text(story.actionLabel),actionUrl:text(story.actionUrl),order:Number(story.order)||0,viewed:uniqueIds(story.viewerIds).includes(Number(clientId))}}
