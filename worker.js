...
function recoverySql(store){if(typeof store==='function')return store;return store?.DATABASE_URL?neon(store.DATABASE_URL):null}
...
async function ensureProtocolTable(store){
  ...
  const sql=recoverySql(store);if(!sql)throw Object.assign(new Error('Banco de protocolos não configurado.'),{statusCode:503});
...
async function createProtocolRecord(store,{clientId=null,category='Atendimento',subject='Atendimento',source='provedor-plus',status='Aberto',createdByUserId=null,createdByName='',details={}}={}){
  ...
  const sql=recoverySql(store),rows=await sql`INSERT INTO pp_protocols (client_id,category,subject,source,status,created_by_user_id,created_by_name,details) VALUES (${client},${cat},${title},${origin},${state},${userId},${name||null},${raw}::jsonb) RETURNING *`;
  const created=Array.isArray(rows)?rows[0]:null;if(!created?.id)throw Object.assign(new Error('Não foi possível gerar o protocolo do atendimento.'),{statusCode:500});
  const code=protocolCode(created.id,created.created_at),updated=await sql`UPDATE pp_protocols SET protocol=${code} WHERE id=${Number(created.id)} RETURNING *`;return safeProtocol(updated?.[0]||{...created,protocol:code});
}
...
async function listProtocolRecords(store,clientId=null,limit=50){
  ...
  const sql=recoverySql(store),rows=client?await sql`SELECT * FROM pp_protocols WHERE client_id=${client} ORDER BY created_at DESC LIMIT ${safeLimit}`:await sql`SELECT * FROM pp_protocols ORDER BY created_at DESC LIMIT ${safeLimit}`;return (Array.isArray(rows)?rows:[]).map(safeProtocol);
}
...
async function closeProtocolRecord(store,protocol,status='Concluído'){
  ...
  const sql=recoverySql(store),rows=await sql`UPDATE pp_protocols SET status=${nextStatus},closed_at=now() WHERE protocol=${code} RETURNING *`;if(!rows?.[0])throw Object.assign(new Error('Protocolo não encontrado.'),{statusCode:404});return safeProtocol(rows[0]);
}
...
async function loadState(store){
  ...
  const sql=recoverySql(store);if(!sql)throw Object.assign(new Error('Estado D1 do Provedor Plus não configurado.'),{statusCode:503});const rows=await sql`SELECT value FROM pp_settings WHERE key=${STATE_KEY} LIMIT 1`;return parseStateValue(Array.isArray(rows)?rows[0]?.value:null);
}
...
async function portalClientById(store,clientId){
  ...
  const sql=recoverySql(store);if(!sql)return null;const rows=await sql`SELECT id,name,document,contract_number,plan,plan_id,due_day,status,email,phone,address,city,state,zip_code,router_id,connection_type,pppoe_username,ip,mikrotik_status,mikrotik_last_sync FROM pp_clients WHERE id=${id} LIMIT 1`;return Array.isArray(rows)?rows[0]||null:null;
}
...
async function portalLiveConnection(env,store,client){
  ...
  try{
    ...
    try{const db=d1Db(store);if(db)await db.prepare("UPDATE pp_clients SET ip=COALESCE(NULLIF(?,''),ip),mikrotik_status=?,mikrotik_last_sync=?,updated_at=? WHERE id=?").bind(text(live.ip),live.online?'Online':'Offline',checkedAt,checkedAt,Number(client.id)).run();else{const sql=recoverySql(store);if(sql)await sql`UPDATE pp_clients SET ip=COALESCE(NULLIF(${text(live.ip)},''),ip),mikrotik_status=${live.online?'Online':'Offline'},mikrotik_last_sync=${checkedAt},updated_at=${checkedAt} WHERE id=${Number(client.id)}`}}catch{}
    return connection;
  }catch(error){
