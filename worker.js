// pp-build: 20260907-support-neg1
...
async function paymentPixForSession(env,data){
  const ctx=await portalPaymentContext(env,data),invoice=portalPaymentInvoice(ctx.state,ctx.client,data?.invoiceId),originalCents=invoiceCents(invoice),override=text(invoice?.payment_mode_override).toLowerCase();let provider=override==='pix_mp'?(ctx.banks.ready.mercadoPago?'mercadoPago':''):override==='pix_due'?(ctx.banks.ready.efiPix?'efi':''):ctx.banks.pixProvider,discountCents=cashbackDiscountCents(ctx.state,ctx.client,invoice),amountCents=Math.max(0,originalCents-discountCents);
