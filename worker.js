...
function cashbackDiscountCents(state,client,invoice){const applied=Math.max(0,Math.round(Number(invoice?.cashback_discount_applied_cents)||0));if(applied)return applied;const status=text(invoice?.status).toLowerCase();if(['pago','paid','baixado','cancelado','canceled','renegociado','renegotiated','substituido','substituida'].some(value=>status.includes(value)))return 0;return Math.min(cashbackBalanceCents(state,client),invoiceCents(invoice))}
function applyCashbackDiscount(state,client,invoice){
  const existing=Math.max(0,Math.round(Number(invoice?.cashback_discount_applied_cents)||0));if(existing)return existing;const amount=Math.min(cashbackBalanceCents(state,client),invoiceCents(invoice));if(amount<=0)return 0;const createdAt=new Date().toISOString(),balance=updateCashbackBalance(state,client,cashbackBalanceCents(state,client)-amount,createdAt);if(!balance)return 0;const transactionId=crypto.randomUUID();appendCashbackTransaction(state,{id:transactionId,client_id:Number(client.id),invoice_id:invoice.id,type:'debit',source:'pix_discount',amount_cents:amount,balance_before_cents:balance.before,balance_after_cents:balance.after,reason:`Desconto no Pix da fatura ${invoiceReference(invoice)||invoice.id}`,created_at:createdAt,created_by_name:'Área do Cliente'});Object.assign(invoice,{cashback_discount_applied_cents:amount,cashback_discount_transaction_id:transactionId,cashback_discount_applied_at:createdAt,cashback_discount_status:'applied',cashback_original_cents:invoiceCents(invoice),cashback_pix_amount_cents:Math.max(0,invoiceCents(invoice)-amount)});return amount;
}
function refundCashbackDiscount(state,client,invoice,reason='Pix cancelado ou recusado'){
  const amount=Math.max(0,Math.round(Number(invoice?.cashback_discount_applied_cents)||0));if(!amount||invoice?.cashback_discount_status==='used')return 0;const createdAt=new Date().toISOString(),balance=updateCashbackBalance(state,client,cashbackBalanceCents(state,client)+amount,createdAt);if(!balance)return 0;appendCashbackTransaction(state,{id:crypto.randomUUID(),client_id:Number(client.id),invoice_id:invoice.id,type:'credit',source:'pix_discount_refund',amount_cents:amount,balance_before_cents:balance.before,balance_after_cents:balance.after,reason,created_at:createdAt,created_by_name:'Estorno automático'});Object.assign(invoice,{cashback_discount_applied_cents:0,cashback_discount_transaction_id:'',cashback_discount_status:'refunded',cashback_discount_refunded_at:createdAt,cashback_pix_amount_cents:invoiceCents(invoice)});return amount;
}
function finalizeCashbackDiscount(invoice,paidAt=''){if(Number(invoice?.cashback_discount_applied_cents)>0)Object.assign(invoice,{cashback_discount_status:'used',cashback_discount_used_at:text(paidAt)||new Date().toISOString()})}
...
function mapInvoice(row,client,state){
  ...
  return {
    ...
    cashbackEnabled:rules.enabled&&row?.cashback_eligible!==false&&row?.cashback_enabled!==false,cashbackMode:rules.mode,cashbackRate:effectiveRate,cashbackFixed:rules.fixedCents/100,cashbackRuleLabel:rules.mode==='fixed'?`${brlCents(rules.fixedCents)} por Pix`:`${rules.rate}% do valor pago`,cashbackPending:cashbackPending/100,cashbackBalance:cashbackBalanceCents(state,client)/100,cashbackDiscount:cashbackDiscount/100,cashbackDiscountCents:cashbackDiscount,pixAmount:pixCents/100,pixAmountCents:pixCents
  };
}
...
async function paymentPixForSession(env,data){
  ...
  if(provider==='efi'){
    ...
    let status=text(invoice.bank_status||invoice.status).toLowerCase(),paid=Boolean(text(invoice.paid_at))||['paid','pago','settled','concluida','concluída','concluido','concluído'].some(value=>status.includes(value));if(paid){markPortalInvoicePaid(invoice,'Pix Efí',invoice.paid_at);finalizeCashbackDiscount(invoice,invoice.paid_at);creditPixCashback(ctx.state,ctx.client,invoice,invoice.paid_at);}
      ...
      if(['cancelad','rejeitad','recusad','expired','expirad','removid'].some(value=>status.includes(value)))refundCashbackDiscount(ctx.state,ctx.client,invoice,'Estorno do desconto: Pix Efí cancelado ou expirado');
