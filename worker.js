...
function d1Db(store){return store?.PROVEDOR_DB||(store?.prepare?store:null)}
...
async function decryptBankSettings(env,record){
  ...
  try{
    const key=await bankCryptoKey(env),plain=await crypto.subtle.decrypt({name:'AES-GCM',iv:bankBytes(record.iv)},key,bankBytes(record.data)),parsed=JSON.parse(new TextDecoder().decode(plain));
...
async function ensureProtocolTable(store){
  const db=d1Db(store);
  if(db){
    await db.batch([
      ...
      db.prepare('CREATE INDEX IF NOT EXISTS pp_protocols_category_created_idx ON pp_protocols (category,created_at DESC)')
    ]);
    return;
  }
  throw Object.assign(new Error('Banco D1 de protocolos não configurado.'),{statusCode:503});
}
...
async function createProtocolRecord(store,{clientId=null,category='Atendimento',subject='Atendimento',source='provedor-plus',status='Aberto',createdByUserId=null,createdByName='',details={}}={}){
  await ensureProtocolTable(store);
  const client=Number(clientId)||null,userId=Number(createdByUserId)||null,cat=text(category)||'Atendimento',title=text(subject)||'Atendimento',origin=text(source)||'provedor-plus',state=text(status)||'Aberto',name=text(createdByName),raw=JSON.stringify(details&&typeof details==='object'?details:{}),db=d1Db(store);
  if(db){
    const createdAt=new Date().toISOString(),inserted=await db.prepare('INSERT INTO pp_protocols (client_id,category,subject,source,status,created_by_user_id,created_by_name,details,created_at) VALUES (?,?,?,?,?,?,?,?,?)').bind(client,cat,title,origin,state,userId,name||null,raw,createdAt).run(),id=Number(inserted?.meta?.last_row_id)||0;
    if(!id)throw Object.assign(new Error('Não foi possível gerar o protocolo do atendimento.'),{statusCode:500});
    const code=protocolCode(id,createdAt);await db.prepare('UPDATE pp_protocols SET protocol=? WHERE id=?').bind(code,id).run();const created=await db.prepare('SELECT * FROM pp_protocols WHERE id=? LIMIT 1').bind(id).first();return safeProtocol(created||{id,protocol:code,client_id:client,category:cat,subject:title,source:origin,status:state,created_by_name:name,created_at:createdAt});
  }
  throw Object.assign(new Error('Banco D1 de protocolos não configurado.'),{statusCode:503});
}

async function listProtocolRecords(store,clientId=null,limit=50){
  await ensureProtocolTable(store);
  const safeLimit=Math.max(1,Math.min(200,Math.floor(Number(limit)||50))),client=Number(clientId)||0,db=d1Db(store);
  if(db){const rows=client?await d1Rows(db.prepare('SELECT * FROM pp_protocols WHERE client_id=? ORDER BY created_at DESC LIMIT ?').bind(client,safeLimit)):await d1Rows(db.prepare('SELECT * FROM pp_protocols ORDER BY created_at DESC LIMIT ?').bind(safeLimit));return rows.map(safeProtocol)}
  throw Object.assign(new Error('Banco D1 de protocolos não configurado.'),{statusCode:503});
}

async function closeProtocolRecord(store,protocol,status='Concluído'){
  await ensureProtocolTable(store);
  const code=text(protocol);if(!code)throw Object.assign(new Error('Protocolo inválido.'),{statusCode:400});const nextStatus=text(status)||'Concluído',db=d1Db(store);
  if(db){const closedAt=new Date().toISOString(),result=await db.prepare('UPDATE pp_protocols SET status=?,closed_at=? WHERE protocol=?').bind(nextStatus,closedAt,code).run();if(!(Number(result?.meta?.changes)||0))throw Object.assign(new Error('Protocolo não encontrado.'),{statusCode:404});const row=await db.prepare('SELECT * FROM pp_protocols WHERE protocol=? LIMIT 1').bind(code).first();return safeProtocol(row)}
  throw Object.assign(new Error('Banco D1 de protocolos não configurado.'),{statusCode:503});
}
...
async function handleProtocols(request,env){
  ...
  try{
    const user=await requirePanelUser(request,env);if(!env?.PROVEDOR_DB)throw Object.assign(new Error('Banco D1 dos protocolos não configurado.'),{statusCode:503});
    let body={};try{body=await request.json()}catch{}
    const action=text(body?.action),data=body?.data||{};let result;
    if(action==='create')result=await createProtocolRecord(env,{clientId:data.clientId,category:data.category,subject:data.subject,source:'provedor-plus',status:data.status||'Aberto',createdByUserId:user?.id,createdByName:user?.name,details:data.details});
    else if(action==='list')result=await listProtocolRecords(env,data.clientId,data.limit);
    else if(action==='close')result=await closeProtocolRecord(env,data.protocol,data.status||'Concluído');
...
async function refreshBankClientRequest(request,env){
  if(!env?.PROVEDOR_DB)return request;
  ...
  const fresh=await env.PROVEDOR_DB.prepare('SELECT id,name,document,contract_number,plan,plan_id,due_day,status,email,phone,address,city,state,zip_code,updated_at FROM pp_clients WHERE id=? LIMIT 1').bind(clientId).first();
...
async function loadState(store){
  const db=d1Db(store);if(db){const row=await db.prepare('SELECT value FROM pp_settings WHERE key=? LIMIT 1').bind(STATE_KEY).first();return parseStateValue(row?.value)}
...
async function portalClientById(store,clientId){
  const db=d1Db(store),id=Number(clientId);
  if(db)return db.prepare('SELECT id,name,document,contract_number,plan,plan_id,due_day,status,email,phone,address,city,state,zip_code,router_id,connection_type,pppoe_username,ip,mikrotik_status,mikrotik_last_sync FROM pp_clients WHERE id=? LIMIT 1').bind(id).first();
  return null;
}
...
function normalizedBilling(value){return text(value).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')}
function invoiceMonthLabel(row){
  const months=['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
  for(const value of [row?.reference,row?.competency,row?.competence,row?.month,row?.period,row?.due_date,row?.dueDate]){
    const raw=text(value);let match=raw.match(/^(\d{4})-(\d{2})(?:-|$)/);if(match){const month=Number(match[2]);if(month>=1&&month<=12)return `${months[month-1]}/${match[1]}`}
    match=raw.match(/^(\d{2})\/(\d{4})$/);if(match){const month=Number(match[1]);if(month>=1&&month<=12)return `${months[month-1]}/${match[2]}`}
  }
  return '';
}
function invoiceBillingMeta(row){
  const origin=normalizedBilling(row?.billing_origin),type=normalizedBilling(row?.billing_type),description=normalizedBilling(row?.description),numberPart=Math.max(0,Math.floor(Number(row?.installment_number)||0)),totalPart=Math.max(0,Math.floor(Number(row?.installment_total)||0)),installment=numberPart&&totalPart?` • Parcela ${numberPart} de ${totalPart}`:'',month=invoiceMonthLabel(row),monthly=origin.includes('monthly')||type.includes('mensalidade')||description.includes('mensalidade'),negotiation=Boolean(text(row?.negotiation_id))||origin.includes('negotiat')||origin.includes('renegoci')||type.includes('renegoci')||type.includes('negociacao')||description.includes('renegoci')||description.includes('acordo');
  if(origin.includes('monthly_auto_combined')||(monthly&&negotiation))return {kind:'monthly-negotiation',label:`Mensalidade + Negociação${installment}`,detail:text(row?.description),installmentNumber:numberPart,installmentTotal:totalPart};
  if(negotiation){const suffix=installment||(origin.includes('negotiation_entry')?' • Entrada':'');return {kind:'negotiation',label:`Negociação${suffix}`,detail:text(row?.description),installmentNumber:numberPart,installmentTotal:totalPart}}
  if(origin==='due_date_change'||type.includes('ajuste proporcional')||description.includes('ajuste proporcional')||description.includes('alteracao de vencimento'))return {kind:'due-date-adjustment',label:'Ajuste proporcional de vencimento',detail:text(row?.description),installmentNumber:numberPart,installmentTotal:totalPart};
  if(origin==='first_prorated'||type.includes('proporcional')||description.includes('proporcional'))return {kind:'proportional',label:'Cobrança proporcional',detail:text(row?.description),installmentNumber:numberPart,installmentTotal:totalPart};
  if(type.includes('instalacao')||description.includes('taxa de instalacao')||description.includes('instalacao'))return {kind:'installation',label:'Taxa de instalação',detail:text(row?.description),installmentNumber:numberPart,installmentTotal:totalPart};
  if(monthly)return {kind:'monthly',label:`Mensalidade${month?` • ${month}`:''}`,detail:text(row?.description),installmentNumber:numberPart,installmentTotal:totalPart};
  return {kind:'charge',label:'Cobrança',detail:text(row?.description),installmentNumber:numberPart,installmentTotal:totalPart};
}
function portalTodayKey(){const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'America/Sao_Paulo',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()),map={};for(const part of parts)map[part.type]=part.value;return `${map.year}-${map.month}-${map.day}`}
function portalDateFromKey(value){const match=text(value).slice(0,10).match(/^(\d{4})-(\d{2})-(\d{2})$/);return match?new Date(Date.UTC(Number(match[1]),Number(match[2])-1,Number(match[3]),12)):null}
function portalOverdueDays(value){const due=portalDateFromKey(value),today=portalDateFromKey(portalTodayKey());if(!due||!today||due>=today)return 0;return Math.floor((today-due)/86400000)}
...
function cashbackContractId(state,client,source=null){
  const explicit=(typeof source==='string'||typeof source==='number')?text(source):text(source?.contract_id);
  if(explicit&&explicit!=='primary')return explicit;
  if(source&&typeof source==='object'){
    const contractNumber=text(source?.contract_number);
    if(contractNumber){const found=(Array.isArray(state?.client_contracts)?state.client_contracts:[]).find(item=>Number(item?.client_id)===Number(client.id)&&text(item?.contract_number)===contractNumber);if(found)return text(found.id)}
  }
  return '';
}
function cashbackAccount(state,client,source=null){
  const contractId=cashbackContractId(state,client,source);
  if(contractId){const contracts=Array.isArray(state?.client_contracts)?state.client_contracts:[],position=contracts.findIndex(item=>Number(item?.client_id)===Number(client.id)&&text(item?.id)===contractId);return {contractId,kind:'contract',position,record:position>=0?contracts[position]:null}}
  const clients=Array.isArray(state?.clients)?state.clients:[],position=clients.findIndex(item=>Number(item?.id)===Number(client.id));return {contractId:'',kind:'client',position,record:position>=0?clients[position]:stateClient(state,client)};
}
function cashbackBalanceCents(state,client,source=null){
  const account=cashbackAccount(state,client,source),local=account.record;if(!local)return 0;if(local?.cashback_balance_cents!==undefined&&local?.cashback_balance_cents!==null){const direct=Number(local.cashback_balance_cents);if(Number.isFinite(direct))return Math.max(0,Math.round(direct))}const amount=Number(local?.cashback_balance);return Number.isFinite(amount)?Math.max(0,Math.round(amount*100)):0;
}
function updateCashbackBalance(state,client,nextCents,updatedAt=new Date().toISOString(),source=null){
  const account=cashbackAccount(state,client,source);if(!account.record||account.position<0)return null;const local={...account.record},before=cashbackBalanceCents(state,client,source),after=Math.max(0,Math.round(Number(nextCents)||0));local.cashback_balance_cents=after;local.cashback_balance=after/100;local.cashback_updated_at=updatedAt;if(account.kind==='contract'){const contracts=[...(Array.isArray(state.client_contracts)?state.client_contracts:[])];contracts[account.position]=local;state.client_contracts=contracts}else{const clients=[...(Array.isArray(state.clients)?state.clients:[])];clients[account.position]=local;state.clients=clients}return {before,after,contractId:account.contractId};
}
...
function cashbackReservedCents(state,client,scope=null){const contractId=cashbackContractId(state,client,scope),excludeId=scope&&typeof scope==='object'&&scope?.id!==undefined&&scope?.id!==null?String(scope.id):'';return (Array.isArray(state?.invoices)?state.invoices:[]).filter(row=>Number(row?.client_id)===Number(client.id)&&cashbackContractId(state,client,row)===contractId&&(excludeId===''||String(row?.id)!==excludeId)).reduce((sum,row)=>sum+cashbackPendingReservation(row),0)}
function cashbackAvailableCents(state,client,invoice=null){return Math.max(0,cashbackBalanceCents(state,client,invoice)-cashbackReservedCents(state,client,invoice))}
function cashbackDiscountCents(state,client,invoice){if(invoice?.cashback_eligible===false||invoice?.cashback_enabled===false)return 0;const applied=Math.max(0,Math.round(Number(invoice?.cashback_discount_applied_cents)||0));if(applied)return applied;const status=text(invoice?.status).toLowerCase();if(['pago','paid','baixado','cancelado','canceled','renegociado','renegotiated','substituido','substituida'].some(value=>status.includes(value)))return 0;return Math.min(cashbackAvailableCents(state,client,invoice),invoiceCents(invoice))}
...
function finalizeCashbackDiscount(state,client,invoice,paidAt=''){
  const amount=Math.max(0,Math.round(Number(invoice?.cashback_discount_applied_cents)||0));if(!amount)return 0;const status=text(invoice?.cashback_discount_status).toLowerCase(),usedAt=text(paidAt)||new Date().toISOString();if(status==='used')return amount;if(status==='applied'){Object.assign(invoice,{cashback_discount_status:'used',cashback_discount_used_at:usedAt});return amount}const contractId=cashbackContractId(state,client,invoice),before=cashbackBalanceCents(state,client,invoice),balance=updateCashbackBalance(state,client,before-amount,usedAt,invoice);if(!balance)return 0;const transactionId=crypto.randomUUID();appendCashbackTransaction(state,{id:transactionId,client_id:Number(client.id),contract_id:contractId||null,invoice_id:invoice.id,type:'debit',source:'pix_discount',amount_cents:amount,balance_before_cents:balance.before,balance_after_cents:balance.after,reason:`Desconto usado no Pix da fatura ${invoiceReference(invoice)||invoice.id}`,created_at:usedAt,payment_at:usedAt,created_by_name:'Pagamento Pix confirmado'});Object.assign(invoice,{cashback_discount_transaction_id:transactionId,cashback_discount_status:'used',cashback_discount_used_at:usedAt});return amount;
}
...
function cashbackPortalData(state,client,scope=null){
  const rules=cashbackRules(state),account=cashbackAccount(state,client,scope),local=account.record||stateClient(state,client),plan=state?.plans?.find?.(item=>Number(item?.id)===Number(local?.plan_id||client.plan_id)),planCents=number(local?.custom_monthly_cents??plan?.price_cents),planValue=number(plan?.price??plan?.amount),nextBaseCents=planCents!==null?Math.max(0,Math.round(planCents)):(planValue!==null?Math.max(0,Math.round(planValue*100)):0),nextCashbackCents=rules.enabled?(rules.mode==='fixed'?rules.fixedCents:Math.max(0,Math.round(nextBaseCents*rules.rate/100))):0,ruleLabel=rules.enabled?(rules.mode==='fixed'?`${brlCents(rules.fixedCents)} por Pix`:`${rules.rate}% do valor pago`):'Cashback desativado',balanceCents=cashbackBalanceCents(state,client,scope),reservedCents=cashbackReservedCents(state,client,scope),availableCents=Math.max(0,balanceCents-reservedCents),contractId=account.contractId,statement=(Array.isArray(state.cashback_transactions)?state.cashback_transactions:[]).filter(item=>Number(item?.client_id)===Number(client.id)&&(contractId?text(item?.contract_id)===contractId:!text(item?.contract_id))).sort((a,b)=>String(b?.created_at||'').localeCompare(String(a?.created_at||''))).slice(0,200).map(item=>({id:text(item?.id),invoiceId:item?.invoice_id??null,contractId:text(item?.contract_id),type:item?.type==='debit'?'debit':'credit',source:text(item?.source),description:text(item?.reason)||'Movimentação de cashback',amountCents:Math.max(0,Math.round(Number(item?.amount_cents)||0)),balanceAfterCents:Math.max(0,Math.round(Number(item?.balance_after_cents)||0)),createdAt:text(item?.created_at),createdBy:text(item?.created_by_name)}));
  return {contractId:contractId||'primary',balanceCents,balance:balanceCents/100,reservedCents,reserved:reservedCents/100,availableCents,available:availableCents/100,enabled:rules.enabled,mode:rules.mode,rate:rules.rate,fixedCents:rules.fixedCents,fixed:rules.fixedCents/100,ruleLabel,nextBaseCents,nextCashbackCents,nextCashback:nextCashbackCents/100,statement,totalCreditsCents:statement.filter(item=>item.type==='credit').reduce((sum,item)=>sum+item.amountCents,0),totalDebitsCents:statement.filter(item=>item.type==='debit').reduce((sum,item)=>sum+item.amountCents,0)};
}
...
function pixCashbackPaymentConfirmed(invoice){const provider=text(invoice?.bank_provider).toLowerCase(),status=normalizedBilling(invoice?.bank_status);if(provider==='mercadopago')return ['approved','paid'].includes(status);if(provider==='efi')return ['paid','pago','settled','concluida','concluido','liquidada','liquidado'].includes(status);return false;}
function creditPixCashback(state,client,invoice,paidAt=''){
  if(!pixCashbackPaymentConfirmed(invoice)||!pixCashbackEligible(invoice)||invoice?.cashback_eligible===false||invoice?.cashback_enabled===false)return false;
  ...
  const contractId=cashbackContractId(state,client,invoice),before=cashbackBalanceCents(state,client,invoice),createdAt=new Date().toISOString(),paymentAt=text(paidAt)||createdAt,transactionId=crypto.randomUUID(),rate=rules.mode==='fixed'?rules.fixedCents:rules.rate,balance=updateCashbackBalance(state,client,before+amountCents,createdAt,invoice);if(!balance)return false;
  const transaction={id:transactionId,client_id:Number(client.id),contract_id:contractId||null,invoice_id:invoice.id,type:'credit',source:'pix_paid',amount_cents:amountCents,balance_before_cents:balance.before,balance_after_cents:balance.after,reason:`Cashback automático do Pix da fatura ${invoiceReference(invoice)||invoice.id}`,created_at:createdAt,payment_at:paymentAt,created_by_name:'Pagamento Pix automático'};
...
function mapInvoice(row,client,state){
  const late=invoiceLateBreakdown(row,state),baseCents=late.baseCents,cents=late.totalCents,total=brlCents(cents),originalTotal=brlCents(baseCents),meta=invoiceBillingMeta(row),planName=text(client.plan_name||client.plan||state?.plans?.find?.(p=>Number(p?.id)===Number(client.plan_id))?.name)||'Serviço de internet',company=state?.settings||state?.company||{},rules=cashbackRules(state),cashbackPending=cashbackAmountCents(state,row),effectiveRate=rules.mode==='fixed'&&baseCents>0?Math.round(cashbackPending/baseCents*10000)/100:rules.rate,cashbackDiscount=cashbackDiscountCents(state,client,row),pixCents=Math.max(0,cents-cashbackDiscount),cashbackAvailable=cashbackAvailableCents(state,client,row),rawStatus=text(row?.status)||'Pendente',displayStatus=late.daysOverdue>0&&!portalPaymentInactiveStatus(rawStatus)?'Vencida':rawStatus;
  ...
  const digitableLine=text(row?.bank_digitable_line||row?.digitable_line||row?.linha_digitavel),fineTotal=brlCents(late.fineCents),interestTotal=brlCents(late.interestCents),lateFeeText=late.feesCents?`${late.daysOverdue} dia(s) em atraso · Multa ${fineTotal} · Juros ${interestTotal}`:'Sem encargos por atraso',moraFine=late.feesCents?`Multa ${fineTotal} + Juros ${interestTotal}`:'R$ 0,00';
  return {
    id:row?.id??null,reference:invoiceReference(row),billingKind:meta.kind,billingLabel:meta.label,billingTitle:meta.label,billingDetail:meta.detail,installmentNumber:meta.installmentNumber,installmentTotal:meta.installmentTotal,dueDate:formatDate(row?.due_date||row?.dueDate),dueDateRaw:text(row?.due_date||row?.dueDate),
    total,totalNumber:cents/100,amountCents:cents,originalAmount:baseCents/100,originalAmountCents:baseCents,originalTotal,updatedAmount:cents/100,updatedAmountCents:cents,updatedTotal:total,amountBeforeDiscount:cents/100,daysOverdue:late.daysOverdue,chargeDays:late.chargeDays,fineAmount:late.fineCents/100,fineCents:late.fineCents,fineTotal,interestAmount:late.interestCents/100,interestCents:late.interestCents,interestTotal,lateFeeText,moraFine,chargedAmount:total,status:displayStatus,serviceName:meta.label,planName,serviceAmount:originalTotal,serviceAmountRaw:originalTotal,subtotal:originalTotal,quantity:'1',unitAmount:originalTotal,
    ...
    cashbackEnabled:rules.enabled&&row?.cashback_eligible!==false&&row?.cashback_enabled!==false,cashbackMode:rules.mode,cashbackRate:effectiveRate,cashbackFixed:rules.fixedCents/100,cashbackRuleLabel:rules.mode==='fixed'?`${brlCents(rules.fixedCents)} por Pix`:`${rules.rate}% do valor pago`,cashbackPending:cashbackPending/100,cashbackBalance:cashbackBalanceCents(state,client,row)/100,cashbackAvailable:cashbackAvailable/100,cashbackDiscount:cashbackDiscount/100,cashbackDiscountCents:cashbackDiscount,pixAmount:pixCents/100,pixAmountCents:pixCents
  };
}
...
async function portalConnectionContext(env,data){
  ...
  const session=await verifyPortalSession(data?.session,env),client=await portalClientById(env,session.clientId);
  if(!client)throw Object.assign(new Error('Cliente da sessão não foi encontrado.'),{statusCode:404});
  const state=await loadState(env);if(repairLegacyPendingCashback(state,client))await saveState(env,state);return {session,sql:env,client,state};
}
async function portalLiveConnection(env,store,client){
...
async function portalSnapshot(client,state,env,sessionToken='',connectionOverride=null,{includeProtocols=true}={}){
  ...
  let protocols=[];
  if(includeProtocols)try{protocols=await listProtocolRecords(env,client.id,20)}catch{}
  return {
...
async function negotiationOptionsForSession(env,data){
  ...
  const session=await verifyPortalSession(data?.session,env),client=await portalClientById(env,session.clientId);
  if(!client)throw Object.assign(new Error('Cliente da sessão não foi encontrado.'),{statusCode:404});
  const state=await loadState(env),preview=buildNegotiationOptions(state,client,Array.isArray(data?.invoiceIds)?data.invoiceIds:undefined);
...
async function negotiateForSession(env,data){
  if(!env?.PROVEDOR_DB)throw Object.assign(new Error('Banco D1 do Provedor Plus não configurado.'),{statusCode:503});
  const session=await verifyPortalSession(data?.session,env),sql=env,client=await portalClientById(env,session.clientId);
  if(!client)throw Object.assign(new Error('Cliente da sessão não foi encontrado.'),{statusCode:404});
  const state=await loadState(env),preview=buildNegotiationOptions(state,client,Array.isArray(data?.invoiceIds)?data.invoiceIds:[]),rules=preview.rules;
  ...
  const protocol=await createProtocolRecord(env,{clientId:client.id,category:'Negociação',subject:'Negociação automática de débitos',source:'area-cliente',status:'Concluído',createdByName:`Área do Cliente · ${text(client.name)}`,details:{agreementId,originalInvoiceIds:[...preview.selectedInvoiceIds],originalAmountCents:preview.originalCents,discountCents:option.discountCents,negotiatedAmountCents:option.totalCents,installmentTotal:option.installments,newInvoiceIds:replacementIds}});
...
async function dbHealth(env){
  if(!env?.PROVEDOR_DB)return {configured:false,connected:false,protocolsReady:false};
  try{
    const row=await env.PROVEDOR_DB.prepare('SELECT 1 AS ok').first();await ensureProtocolTable(env);
    return {configured:true,connected:Number(row?.ok)===1,protocolsReady:true};
  }catch(error){
    return {configured:true,connected:false,protocolsReady:false,error:'Falha de conexão com o D1'};
  }
}
...
async function nativePortalLogin(env,data){
  ...
  const db=env.PROVEDOR_DB,contractDigits=digits(contract),hasDocument=document?1:0,hasContract=contract?1:0,clean="replace(replace(replace(replace(replace(replace(COALESCE(%s,''),'.',''),'-',''),'/',''),'(',''),')',''),' ','')";
  const clients=await d1Rows(db.prepare(`SELECT id,name,document,contract_number,plan,plan_id,due_day,status,email,phone,address,city,state,zip_code,router_id,connection_type,pppoe_username,ip,mikrotik_status,mikrotik_last_sync FROM pp_clients WHERE (?=0 OR ${clean.replace('%s','document')}=?) AND (?=0 OR contract_number=? OR ${clean.replace('%s','contract_number')}=?) LIMIT 1`).bind(hasDocument,document,hasContract,contract,contractDigits));
  ...
  const state=await loadState(env);if(repairLegacyPendingCashback(state,client))await saveState(env,state);
...
async function portalPaymentContext(env,data){
  if(!env?.PROVEDOR_DB)throw Object.assign(new Error('Banco D1 do Provedor Plus não configurado.'),{statusCode:503});
  const session=await verifyPortalSession(data?.session,env),client=await portalClientById(env,session.clientId);if(!client)throw Object.assign(new Error('Cliente da sessão não foi encontrado.'),{statusCode:404});
...
async function paymentPixForSession(env,data){
  const ctx=await portalPaymentContext(env,data),invoice=portalPaymentInvoice(ctx.state,ctx.client,data?.invoiceId),late=invoiceLateBreakdown(invoice,ctx.state),meta=invoiceBillingMeta(invoice),originalCents=late.baseCents,payableCents=late.totalCents,override=text(invoice?.payment_mode_override).toLowerCase();let provider=override==='pix_mp'?(ctx.banks.ready.mercadoPago?'mercadoPago':''):override==='pix_due'?(ctx.banks.ready.efiPix?'efi':''):ctx.banks.pixProvider,discountCents=cashbackDiscountCents(ctx.state,ctx.client,invoice),amountCents=Math.max(0,payableCents-discountCents);
  if(override==='pix_mp'&&!provider)throw Object.assign(new Error('Este acordo foi definido para Pix Mercado Pago, mas o Mercado Pago não está disponível no momento.'),{statusCode:409});
  if(override==='pix_due'&&!provider)throw Object.assign(new Error('Este acordo foi definido para Pix Efí, mas a Efí não está disponível no momento.'),{statusCode:409});
  if(amountCents<=0&&discountCents>0){discountCents=applyCashbackDiscount(ctx.state,ctx.client,invoice);markPortalInvoicePaid(invoice,'Cashback',new Date().toISOString());finalizeCashbackDiscount(ctx.state,ctx.client,invoice,invoice.paid_at);Object.assign(invoice,{bank_provider:'cashback',bank_status:'paid',bank_status_detail:'cashback_full',payment_origin:'area-cliente',cashback_pix_amount_cents:0});await saveState(ctx.sql,ctx.state);const portal=await portalSnapshot(ctx.client,ctx.state,env,ctx.session.token);return {provider:'cashback',providerLabel:'Cashback',paymentId:`cashback-${invoice.id}`,status:'approved',state:'Pago',message:'Fatura quitada integralmente com o saldo de cashback.',qrCode:'',qrCodeBase64:'',paymentUrl:'',billingLabel:meta.label,originalAmount:originalCents/100,daysOverdue:late.daysOverdue,fineAmount:late.fineCents/100,interestAmount:late.interestCents/100,updatedAmount:payableCents/100,cashbackDiscount:discountCents/100,amount:0,portal};}
  ...
  if(provider==='efi'){
    ...
    }else if(text(invoice.bank_provider)==='efi'&&existingDetail==='efi_pix_cobv'&&text(invoice.bank_charge_id)){
      try{
        const synced=await bankProxyAsService(env,ctx.sql,{action:'sync',invoice:bankInvoice(invoice,ctx.client,ctx.state),efi:ctx.banks.secrets.efi,mercadoPago:ctx.banks.secrets.mercadoPago});Object.assign(invoice,synced||{});const currentStatus=text(invoice.bank_status||invoice.status).toLowerCase(),paid=Boolean(text(invoice.paid_at))||Boolean(text(synced?.paidAt))||['paid','pago','settled','concluida','concluída','concluido','concluído'].some(value=>currentStatus.includes(value)),closed=['cancelad','canceled','cancelled','expired','expirad','removid','rejeitad','recusad'].some(value=>currentStatus.includes(value));
        if(paid){const paidAt=text(synced?.paidAt||invoice.paid_at);markPortalInvoicePaid(invoice,'Pix Efí',paidAt);finalizeCashbackDiscount(ctx.state,ctx.client,invoice,paidAt);creditPixCashback(ctx.state,ctx.client,invoice,paidAt)}
        else if(late.feesCents>0&&!closed){await bankProxyAsService(env,ctx.sql,{action:'cancel',invoice:bankInvoice(invoice,ctx.client,ctx.state),efi:ctx.banks.secrets.efi,mercadoPago:ctx.banks.secrets.mercadoPago});const confirmed=await bankProxyAsService(env,ctx.sql,{action:'sync',invoice:bankInvoice(invoice,ctx.client,ctx.state),efi:ctx.banks.secrets.efi,mercadoPago:ctx.banks.secrets.mercadoPago});Object.assign(invoice,confirmed||{});const confirmedStatus=text(invoice.bank_status||'').toLowerCase(),paidAfterCancel=Boolean(text(confirmed?.paidAt))||['paid','pago','settled','concluida','concluída','concluido','concluído'].some(value=>confirmedStatus.includes(value));if(paidAfterCancel){const paidAt=text(confirmed?.paidAt);markPortalInvoicePaid(invoice,'Pix Efí',paidAt);finalizeCashbackDiscount(ctx.state,ctx.client,invoice,paidAt);creditPixCashback(ctx.state,ctx.client,invoice,paidAt)}else if(['cancelad','canceled','cancelled','expired','expirad','removid','rejeitad','recusad'].some(value=>confirmedStatus.includes(value))){refundCashbackDiscount(ctx.state,ctx.client,invoice,'Reserva liberada após atualização do Pix Efí vencido');clearPortalPixBankFields(invoice)}else throw Object.assign(new Error('Efí Pix: não foi possível atualizar o valor da cobrança vencida. Nenhum novo Pix foi gerado.'),{statusCode:409});}
      }catch(error){if(Number(error?.statusCode))throw error;}
    }
    let status=text(invoice.bank_status||invoice.status).toLowerCase(),paid=Boolean(text(invoice.paid_at))||['paid','pago','settled','concluida','concluída','concluido','concluído'].some(value=>status.includes(value));if(paid){markPortalInvoicePaid(invoice,'Pix Efí',invoice.paid_at);finalizeCashbackDiscount(ctx.state,ctx.client,invoice,invoice.paid_at);creditPixCashback(ctx.state,ctx.client,invoice,invoice.paid_at);}
      ...
      discountCents=cashbackDiscountCents(ctx.state,ctx.client,invoice);amountCents=Math.max(0,payableCents-discountCents);const needsDiscountedPix=discountCents>0&&!Number(invoice.cashback_discount_applied_cents);
      if(!text(invoice.bank_pix_code)||needsDiscountedPix){const source={...invoice,due_date:late.daysOverdue>0?portalTodayKey():invoice.due_date,amount_cents:amountCents,amount:amountCents/100,billing_type:'Pix com vencimento',description:meta.label},remote=await bankProxyAsService(env,ctx.sql,{action:'issue',provider:'efi',invoice:bankInvoice(source,ctx.client,ctx.state),client:bankClient(ctx.client,ctx.state),efi:ctx.banks.secrets.efi,mercadoPago:ctx.banks.secrets.mercadoPago,pixAutoRecord:null});Object.assign(invoice,remote||{});if(discountCents>0&&!Number(invoice.cashback_discount_applied_cents))discountCents=applyCashbackDiscount(ctx.state,ctx.client,invoice);}
    }
    ...
    discountCents=cashbackDiscountCents(ctx.state,ctx.client,invoice);amountCents=Math.max(0,payableCents-discountCents);
    if(mp&&!mpPaid(mp.status)&&!mpRejected(mp.status)){const remoteCents=Math.max(0,Math.round(Number(mp?.transaction_amount||0)*100)),mustReplace=remoteCents!==amountCents||(discountCents>0&&!Number(invoice.cashback_discount_applied_cents));if(mustReplace){try{await mercadoPagoRequest(ctx.vault,`/v1/payments/${encodeURIComponent(mp.id)}`,{method:'PUT',body:{status:'cancelled'},idempotencyKey:crypto.randomUUID()});mp=null}catch{throw Object.assign(new Error('Mercado Pago: não foi possível atualizar o Pix para o valor atual com juros. Tente novamente.'),{statusCode:409})}}}
    if(!mp){const name=text(ctx.client.name),parts=name.split(/\s+/).filter(Boolean),first=parts.shift()||name,last=parts.join(' ')||first,payload={transaction_amount:amountCents/100,description:meta.label,payment_method_id:'pix',external_reference:`PP-INV-${invoice.id}-CLI-${ctx.client.id}`,payer:{email,first_name:first,last_name:last,identification:{type:digits(ctx.client.document).length===14?'CNPJ':'CPF',number:digits(ctx.client.document)}}};mp=await mercadoPagoRequest(ctx.vault,'/v1/payments',{method:'POST',body:payload,idempotencyKey:crypto.randomUUID()});if(discountCents>0&&!Number(invoice.cashback_discount_applied_cents))discountCents=applyCashbackDiscount(ctx.state,ctx.client,invoice);}
...
async function paymentCardForSession(env,data){
  ...
  const payload={token:text(pd.token),transaction_amount:(late.totalCents+CARD_PAYMENT_FEE_CENTS)/100,installments:Math.max(1,Number(pd.installments)||1),payment_method_id:text(pd.payment_method_id),description:meta.label,external_reference:`PP-INV-${invoice.id}-CLI-${ctx.client.id}`,payer};if(pd.issuer_id)payload.issuer_id=String(pd.issuer_id);
  ...
  await saveState(ctx.sql,ctx.state);const portal=await portalSnapshot(ctx.client,ctx.state,env,ctx.session.token);return {provider:'mercadoPago',providerLabel:'Mercado Pago',paymentId:text(mp.id),status:text(mp.status),statusDetail:text(mp.status_detail),message:text(mp.status_detail),billingLabel:meta.label,originalAmount:late.baseCents/100,daysOverdue:late.daysOverdue,fineAmount:late.fineCents/100,interestAmount:late.interestCents/100,updatedAmount:late.totalCents/100,cardFee:CARD_PAYMENT_FEE_CENTS/100,cardFeeCents:CARD_PAYMENT_FEE_CENTS,amount:(late.totalCents+CARD_PAYMENT_FEE_CENTS)/100,portal};
}
async function paymentStatusForSession(env,data){
  ...
  if(provider==='mercadoPago'&&text(invoice.bank_payment_id||data?.paymentId)){
    const paymentId=text(invoice.bank_payment_id||data.paymentId),previousDetail=text(invoice.bank_status_detail),previousOrderId=text(invoice.bank_order_id),previousTicketUrl=text(invoice.bank_ticket_url),mp=await mercadoPagoRequest(ctx.vault,`/v1/payments/${encodeURIComponent(paymentId)}`),environment=text(ctx.vault?.mercadoPago?.environment)||'sandbox',methodId=text(mp?.payment_method_id).toLowerCase(),paymentType=text(mp?.payment_type_id).toLowerCase(),kind=previousDetail==='mercado_pago_card'||['credit_card','debit_card'].includes(paymentType)?'card':methodId==='pix'||previousDetail==='mercado_pago_pix'?'pix':paymentType==='ticket'||methodId==='boleto'||Boolean(previousOrderId)?'boleto':'other',detail=kind==='card'?'mercado_pago_card':kind==='pix'?'mercado_pago_pix':kind==='boleto'?'mercado_pago_boleto':previousDetail||'mercado_pago_payment';
    Object.assign(invoice,mpPaymentFields(mp,detail,environment));if(kind==='boleto'){invoice.bank_order_id=previousOrderId;if(previousTicketUrl&&!text(invoice.bank_ticket_url))invoice.bank_ticket_url=previousTicketUrl;}
    status=text(mp.status);paidAt=text(mp.date_approved||mp.date_last_updated);changed=true;
    if(mpPaid(status)){const method=kind==='card'?'Cartão Mercado Pago':kind==='boleto'?'Boleto Mercado Pago':kind==='pix'?'Pix Mercado Pago':'Mercado Pago';markPortalInvoicePaid(invoice,method,paidAt);if(kind==='pix'){finalizeCashbackDiscount(ctx.state,ctx.client,invoice,paidAt);creditPixCashback(ctx.state,ctx.client,invoice,paidAt);}else refundCashbackDiscount(ctx.state,ctx.client,invoice,`Reserva de cashback liberada: pagamento confirmado por ${method}.`);}
    else if(mpRejected(status)&&kind==='pix')refundCashbackDiscount(ctx.state,ctx.client,invoice,'Estorno do desconto: Pix Mercado Pago cancelado ou recusado');
  }
  else if(provider==='efi'&&text(invoice.bank_charge_id)){
    const detail=text(invoice.bank_status_detail).toLowerCase(),kind=['efi_pix_cobv','efi_pix_auto','efi_bolix_pix'].includes(detail)?'pix':detail==='efi_carnet'?'carnet':'boleto',remote=await bankProxyAsService(env,ctx.sql,{action:'sync',invoice:bankInvoice(invoice,ctx.client,ctx.state),efi:ctx.banks.secrets.efi,mercadoPago:ctx.banks.secrets.mercadoPago});Object.assign(invoice,remote||{});status=text(remote?.bank_status||invoice.bank_status);paidAt=text(remote?.paidAt);changed=true;const normalized=status.toLowerCase(),paid=Boolean(paidAt)||['paid','pago','settled','concluida','concluída','concluido','concluído'].some(value=>normalized.includes(value));
    if(paid){const method=kind==='pix'?'Pix Efí':kind==='carnet'?'Carnê Efí':'Boleto Efí';markPortalInvoicePaid(invoice,method,paidAt);if(kind==='pix'){finalizeCashbackDiscount(ctx.state,ctx.client,invoice,paidAt);creditPixCashback(ctx.state,ctx.client,invoice,paidAt);}else refundCashbackDiscount(ctx.state,ctx.client,invoice,`Reserva de cashback liberada: pagamento confirmado por ${method}.`);}
    else if(kind==='pix'&&['cancelad','rejeitad','recusad','expired','expirad','removid'].some(value=>normalized.includes(value)))refundCashbackDiscount(ctx.state,ctx.client,invoice,'Estorno do desconto: Pix Efí cancelado ou expirado');
  }
...
async function refreshPortalForSession(env,data){
  const ctx=await portalConnectionContext(env,data),contractId=text(data?.contractId)||'primary';
  if(contractId!=='primary')return portalSnapshot(ctx.client,ctx.state,env,ctx.session.token);
  const connection=await portalLiveConnection(env,ctx.sql,ctx.client);return portalSnapshot(ctx.client,ctx.state,env,ctx.session.token,connection);
}
...
async function handleMercadoPagoWebhook(request,env,cors={}){
  ...
  if(!env?.PROVEDOR_DB)throw Object.assign(new Error('Banco D1 do Provedor Plus não configurado.'),{statusCode:503});
  const state=await loadState(env),external=text(payment?.external_reference),match=external.match(/^PP-INV-(.+?)-CLI-(\d+)$/i),invoices=Array.isArray(state?.invoices)?state.invoices:[];
  ...
  const methodId=text(payment?.payment_method_id).toLowerCase(),paymentType=text(payment?.payment_type_id).toLowerCase(),previousDetail=text(invoice.bank_status_detail),previousOrderId=text(invoice.bank_order_id),previousTicketUrl=text(invoice.bank_ticket_url),kind=previousDetail==='mercado_pago_card'||['credit_card','debit_card'].includes(paymentType)?'card':methodId==='pix'||previousDetail==='mercado_pago_pix'?'pix':paymentType==='ticket'||methodId==='boleto'||Boolean(previousOrderId)?'boleto':'other',detail=kind==='card'?'mercado_pago_card':kind==='pix'?'mercado_pago_pix':kind==='boleto'?'mercado_pago_boleto':previousDetail||'mercado_pago_payment',environment=text(mpVault.environment)||'sandbox',paidAt=text(payment?.date_approved||payment?.date_last_updated);
  Object.assign(invoice,mpPaymentFields(payment,detail,environment));if(kind==='boleto'){invoice.bank_order_id=previousOrderId;if(previousTicketUrl&&!text(invoice.bank_ticket_url))invoice.bank_ticket_url=previousTicketUrl;}
  if(mpPaid(payment?.status)){const method=kind==='card'?'Cartão Mercado Pago':kind==='boleto'?'Boleto Mercado Pago':kind==='pix'?'Pix Mercado Pago':'Mercado Pago';markPortalInvoicePaid(invoice,method,paidAt);if(kind==='pix'){finalizeCashbackDiscount(state,client,invoice,paidAt);creditPixCashback(state,client,invoice,paidAt);}else refundCashbackDiscount(state,client,invoice,`Reserva de cashback liberada: pagamento confirmado por ${method}.`);}
  else if(mpRejected(payment?.status)&&kind==='pix')refundCashbackDiscount(state,client,invoice,'Estorno do desconto: Pix Mercado Pago cancelado ou recusado');
